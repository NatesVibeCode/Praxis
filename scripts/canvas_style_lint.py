...
CANVAS_ROOT = ROOT / "Code&DBs" / "Workflow" / "surfaces" / "app" / "src" / "canvas"
TOKENS_FILE = CANVAS_ROOT / "canvas-tokens.css"
STYLE_FILES = [
    CANVAS_ROOT / "canvas-build.css",
    *sorted((CANVAS_ROOT / "style").glob("*.css")),
]
...
RULES = {
    "hex-color": {
        "pattern": HEX_COLOR,
        "message": "direct hex color literals are forbidden outside canvas-tokens.css",
    },
    "rgb-color": {
        "pattern": RGB_COLOR,
        "message": "rgb/rgba/hsla color functions are forbidden outside canvas-tokens.css",
    },
    "important": {
        "pattern": IMPORTANT,
        "message": "!important is forbidden outside canvas-tokens.css",
    },
    "css-var-fallback": {
        "pattern": VAR_FALLBACK,
        "message": "legacy css var fallbacks are forbidden outside canvas-tokens.css",
    },
}
            ...
            json.dumps(
                {
                    ...
                    "errors": [
                        ...
                        "canvas-build style files must be token-first",
                    ],
                    "warnings": [],
                },
                indent=2,
