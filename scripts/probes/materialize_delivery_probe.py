...
def _fake_build_payload(
    ...
    *,
    ...
    materialize_preview: dict[str, Any] | None = None,
    **_kwargs: Any,
) -> dict[str, Any]:
...
def _install_spoofs(stack: ExitStack, conn: _ProbeConn) -> None:
    ...
    import runtime.materialize_cqrs as materialize_cqrs
    ...
    stack.enter_context(patch.object(materialize_cqrs, "recognize_intent", lambda intent, conn, match_limit=5: _FakeRecognition(intent)))
...
def run_probe(*, mode: str = "deterministic") -> dict[str, Any]:
    ...
    from runtime.operations.queries.materialize_preview import (
        CompilePreviewQuery,
    ...
    preview_binding = _binding(
        ...
        operation_name="materialize_preview",
    ...
    materialize_binding = _binding(
        ...
        event_required=True,
        event_type="materialize.committed",
    )
    ...
    bindings = {
        "materialize_preview": preview_binding,
    ...
    try:
        with ExitStack() as stack:
            ...
            stack.enter_context(
                __import__("unittest.mock").mock.patch.object(
                    ...
                    lambda _conn, include_disabled=False, limit=500: [
                        SimpleNamespace(operation_name="materialize_preview"),
            ...
            if preview_receipt["operation_kind"] != "query" or preview_receipt["event_ids"]:
                raise AssertionError("materialize_preview must be a receipt-backed query with no events")
            if not (preview.get("enough_structure") or preview.get("scope_packet", {}).get("gaps")):
                raise AssertionError("materialize_preview returned neither enough_structure nor typed gaps")
            ...
            if "materialize.committed" not in event_types:
                raise AssertionError("materialize.committed authority event was not emitted")
            stages[-1].update(
                {
                    "workflow_id": workflow_id,
