"""Tool: praxis_canvas — legacy alias for Workflow graph authoring.

Single MCP tool that routes to five graph-authoring CQRS operations:
``get_build``, ``compose``, ``suggest_next``, ``mutate_field``, ``launch``.

Mirrors the chat-side legacy ``canvas_*`` registry in ``runtime.chat_tools`` so
...
def _err(message: str, *, action: str, code: str | None = None) -> dict[str, Any]:
    return {
        "ok": False,
        "error": message,
        "error_code": code or f"praxis_canvas.{action}.invalid",
        "action": action,
    }
...
def tool_praxis_canvas(params: dict) -> dict:
    ...
    if action not in _ACTIONS:
        return _err(
            ...
            code="praxis_canvas.action.invalid",
        )
    ...
    "praxis_canvas": (
        tool_praxis_canvas,
        {
            "description": (
                ...
                "praxis_canvas tool name. Five actions over the same workflow build "
                ...
                "EXAMPLES:\n"
                "  Read graph:    praxis_canvas(action='get_build', workflow_id='wf_abc')\n"
                "  Compose:       praxis_canvas(action='compose', intent='Search GH issues, draft summary, notify Slack')\n"
                "  Suggest:       praxis_canvas(action='suggest_next', workflow_id='wf_abc', node_id='node-2')\n"
                "  Edit field:    praxis_canvas(action='mutate_field', workflow_id='wf_abc',\n"
                "                            subpath='nodes/node-1', body={'title':'Webhook v2'})\n"
                "  Launch:        praxis_canvas(action='launch', workflow_id='wf_abc', approved_by='op@team')\n"
            ),
            "inputSchema": {
                "type": "object",
                "properties": {
