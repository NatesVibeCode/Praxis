...
class WorkflowRunner:
    ...
    def _inject_platform_context(self, job: dict) -> str:
        ...
        if touches_infra:
            ...
            lines.append("  praxis install   — install the single Praxis background agent (auto-start on login)")
            lines.append("  praxis status    — show all services, PIDs, ports, connectivity")
            ...
            lines.append("  launchd owns one background item: com.praxis.engine")
            lines.append("  Praxis supervises these internal components:")
            lines.append("    - com.praxis.postgres        (port 5432)")
            lines.append("    - com.praxis.api-server      (port 8420)")
            lines.append("    - com.praxis.workflow-worker (event-bus workflow worker)")
            lines.append("    - com.praxis.scheduler       (60s tick loop)")
            lines.append("  praxis-ctl remains available as a compatibility alias.")
            lines.append("  The launchd plist is generated by praxis-service-manager at install time.")
            lines.append("")
