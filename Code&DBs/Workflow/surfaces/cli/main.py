...
_COMMAND_INDEX_ENTRIES: list[dict[str, str]] = [
    ...
    {"command": "workflow firecheck", "description": "Preflight whether workflow work can actually fire"},
    {"command": "workflow runtime-truth", "description": "Read the runtime truth evidence packet"},
    {"command": "workflow remediation-plan", "description": "Explain allowed remediation for a typed failure"},
    {"command": "workflow remediation-apply", "description": "Apply guarded local runtime remediation"},
    ...
    {"command": "workflow credential <list|onboard|remove>", "description": "Per-sandbox per-provider credential authority — seed credential_tokens for sandbox-spawn auth"},
    ...
    {
        "command": "workflow setup|health|health-map|metrics|events|cache|circuits|slots|params|config|notifications|dashboard|api [routes|--host|--port]|routes|supervisor|capabilities|work",
...
def _workflow_arg_commands() -> dict[str, ArgsCommandHandler]:
    ...
    _ARG_COMMANDS = {
        ...
        "firecheck": _lazy_workflow_args_command("_firecheck_command"),
        "runtime-truth": _lazy_workflow_args_command("_runtime_truth_command"),
        "remediation-plan": _lazy_workflow_args_command("_remediation_plan_command"),
        "remediation-apply": _lazy_workflow_args_command("_remediation_apply_command"),
        ...
        "credential": _lazy_args_command(".commands.credential", "_credential_command"),
...
def _help_text() -> str:
    return "\n".join(
        [
            ...
            "  workflow remediation-plan --failure-type <type>",
            "  workflow remediation-apply --failure-type <type>",
            ...
            "  workflow run|preview|run-status|firecheck|runtime-truth|remediation-plan|remediation-apply|status|active|scheduler|loop|debate|runs|manifest|triggers|retry|cancel|repair|heal|verify|verify-platform|pipeline|proof|queue|diagnose|inspect-job|heartbeat",
