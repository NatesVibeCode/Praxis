 ...
 * graph. Emits a compact SVG in the Moon visual vocabulary:
 *
 *   [trigger chevron?] [ body dots × N ] [status ring]
 *
 * The hollow-circle language matches .moon-graph-node and the
 * MoonStatusRing component so the dashboard row and the canvas read as
 * one product. The rightmost dot carries last-run state — a failed run
 * paints the tail coral (the ONLY hue Moon allows).
 *
...
function statusVar(state: 'ok' | 'active' | 'failed' | 'idle' | 'pending'): string {
  switch (state) {
    case 'ok':
      return 'var(--moon-status-ok, #ffffff)';
    case 'active':
      return 'var(--moon-status-active, #ffffff)';
    case 'failed':
      return 'var(--moon-status-failed, #ff8a6a)';
    case 'idle':
      return 'var(--moon-status-idle, #8b949e)';
    case 'pending':
    default:
      return 'var(--moon-status-pending, #484f58)';
  }
}

export function MoonWorkflowSilhouette({
  nodeCount = 1,
  hasTrigger = false,
  isCron = false,
  lastRunStatus,
  ...
  const neutralStroke = 'var(--moon-status-pending, #484f58)';
  ...
  return (
    <svg
      className="moon-workflow-silhouette"
      width={width}
      height={height}
