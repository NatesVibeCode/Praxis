...
export function CanvasDecisionsPanel() {
  ...
  return (
    <details className="canvas-decisions-panel">
      <summary className="canvas-dock__section-label canvas-decisions-panel__summary">
        Decisions
        {pendingCount > 0 && (
          <span className="canvas-decisions-panel__pending-badge">
            {' '}· {pendingCount} pending review
          </span>
        )}
      </summary>

      <div className="canvas-decisions-panel__controls">
        <select
          className="canvas-decisions-panel__select"
          value={kind}
        ...
        <label className="canvas-decisions-panel__checkbox">
          <input
            type="checkbox"
        ...
        <button
          type="button"
          className="canvas-decisions-panel__refresh"
          onClick={reload}
          disabled={loading}
        >
      ...
      {error && (
        <div className="canvas-decisions-panel__error">scope_clamp fetch failed: {error}</div>
      )}

      <ul className="canvas-decisions-panel__list">
        {filtered.map((row) => {
          ...
          return (
            <li
              ...
              className={`canvas-decision-card${pending ? ' canvas-decision-card--pending' : ''}`}
            >
              <div className="canvas-decision-card__header">
                <div className="canvas-decision-card__title">{row.title}</div>
                <div className="canvas-decision-card__meta">
              ...
              <div className="canvas-decision-card__clamp">
                {pending ? (
                  <div className="canvas-decision-card__clamp-pending">
                  ...
                  <>
                    {appliesTo.length > 0 && (
                      <div className="canvas-decision-card__clamp-block">
                        <div className="canvas-decision-card__clamp-label">Applies to</div>
                        <ul>
                          {appliesTo.map((s, i) => (
                            <li key={`a-${i}`}>{s}</li>
                          ))}
                    ...
                    {doesNotApplyTo.length > 0 && (
                      <div className="canvas-decision-card__clamp-block canvas-decision-card__clamp-block--negative">
                        <div className="canvas-decision-card__clamp-label">Does NOT apply to</div>
                        <ul>
              ...
              <div className="canvas-decision-card__footer">
                <code className="canvas-decision-card__key">{row.decision_key}</code>
                <button
                  type="button"
                  className="canvas-decision-card__expand"
                  onClick={() =>
              ...
              {expanded && (
                <pre className="canvas-decision-card__rationale">{row.rationale}</pre>
              )}
            </li>
          );
        })}
        {filtered.length === 0 && !loading && (
          <li className="canvas-decisions-panel__empty">No decisions matched.</li>
        )}
      </ul>
    </details>
  );
