// Pure presenter: RunDetail -> CanvasBuildViewModel.
// Mirrors the shape of canvasBuildPresenter so CanvasBuildPage can render a
...
import type {
  ...
  GraphLayout,
  LayoutNode,
} from './canvasBuildPresenter';
import { RANK_SPACING, COLUMN_SPACING, NODE_SHAPE_DIMENSIONS, glyphFromLabel, computeLineage } from './canvasBuildPresenter';
...
/**
 * Map a workflow job/run-graph node status to a Canvas ring state.
 * Mirrors runStatusToRingState in canvasBuildPresenter.ts, but exported here
 ...
 * canvasBuildPresenter's TYPE_TOKEN_TO_GLYPH. Same type → same icon.
 */
...
export function presentRun(
  ...
  const empty: CanvasBuildViewModel = {
    nodes: [],
    edges: [],
    dominantPath: [],
    branchBoard: [],
  ...
  // Sibling fan metadata — matches the build presenter so CanvasEdges can
...
function runReleaseStatus(run: RunDetail): CanvasBuildViewModel['release'] {
