...
import type { OrbitEdge, GraphLayout, GateState, GlyphType } from './canvasBuildPresenter';
import { CANVAS_LAYOUT } from './canvasLayout';

const EDGE_STEM = 32;
export const CANVAS_GRAPH_PAD = CANVAS_LAYOUT.canvasPad;
...
export function getEdgeGeometry(
  ...
  const fromHalfWidth = (from.width ?? CANVAS_LAYOUT.nodeWidth) / 2;
  const fromHalfHeight = (from.height ?? CANVAS_LAYOUT.nodeHeight) / 2;
  const toHalfWidth = (to.width ?? CANVAS_LAYOUT.nodeWidth) / 2;
  const toHalfHeight = (to.height ?? CANVAS_LAYOUT.nodeHeight) / 2;
...
export type CanvasGateTone = 'empty' | 'inert' | 'pending' | 'passed' | 'blocked';
...
function gateTone(gateState: GateState): CanvasGateTone {
  switch (gateState) {
...
function stateColor(tone: CanvasGateTone, baseColor: string): string {
  switch (tone) {
    case 'blocked': return 'var(--canvas-state-error, #f85149)';
    case 'passed': return 'var(--canvas-state-success, #3fb950)';
    case 'pending': return 'var(--canvas-state-warning, #d29922)';
    case 'inert':
    case 'empty':
    default:
      return baseColor;
...
export function edgePresentation(
  ...
  const baseColor = isFlowing
    ? 'var(--canvas-fg, #ffffff)'
    : 'var(--canvas-fg-muted, rgba(232, 232, 232, 0.55))';
...
export function CanvasEdges({ edges, layout, selectedEdgeId, onEdgeClick }: CanvasEdgesProps) {
  return (
    <svg
      className="canvas-graph-edges"
      style={{
        position: 'absolute',
        top: 0,
        left: 0,
        width: layout.width + CANVAS_GRAPH_PAD * 2,
        height: layout.height + CANVAS_GRAPH_PAD * 2,
        overflow: 'visible',
        pointerEvents: 'none',
        zIndex: 4,
      }}
    >
      <defs>
        <filter id="canvas-edge-glow" x="-20%" y="-20%" width="140%" height="140%">
      ...
      {edges.map((edge) => {
        ...
        return (
          <g
            key={edge.id}
            filter={isSelected ? 'url(#canvas-edge-glow)' : undefined}
            ...
            {isSelected && (
              <path
                ...
                style={{
                  pointerEvents: 'none',
                  opacity: 0.24,
                  animation: 'canvas-edge-flow 2.5s infinite linear'
                }}
              />
            )}
          </g>
