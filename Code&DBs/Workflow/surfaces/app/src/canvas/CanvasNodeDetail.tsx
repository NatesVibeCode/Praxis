...
import type { OrbitNode, OrbitEdge, DockContent } from './canvasBuildPresenter';
import type {
  BindingLedgerEntry,
  BindingTarget,
  BuildPayload,
...
function WorkflowInspector({
  ...
  if (!summary) {
    return <div className="canvas-dock__empty">Select a node or gate.</div>;
  }
  ...
  return (
    <div className="canvas-dock-workflow" aria-label="Workflow inspector summary">
      <div className="canvas-dock-section-card canvas-dock-section-card--dense">
        <div className="canvas-dock-section-card__header">
          <div className="canvas-dock__section-label">Workflow</div>
          <span className={`canvas-dock-workflow__tone canvas-dock-workflow__tone--${mapTone}`}>
            {summary.readiness}
          </span>
        </div>
        <div className="canvas-dock-workflow__title">{summary.title}</div>
      </div>

      <div className="canvas-dock-workflow__grid">
        <div className="canvas-dock-workflow__row">
          <span>Map</span>
        ...
        <div className={`canvas-dock-workflow__row${summary.disconnected > 0 ? ' canvas-dock-workflow__row--warn' : ''}`}>
      ...
      <div className="canvas-dock-section-card canvas-dock-section-card--dense">
        <div className="canvas-dock__section-label">Data pills</div>
        <div className="canvas-dock-workflow__pills">
          {summary.dataPills.length > 0
            ? summary.dataPills.map((label) => <span key={label}>{label}</span>)
            : <span className="canvas-dock-workflow__pill-empty">none declared</span>}
        </div>
      </div>

      {summary.contextAuthority ? (
        <div className="canvas-dock-section-card canvas-dock-section-card--dense" aria-label="Workflow context authority">
          <div className="canvas-dock-section-card__header">
            <div className="canvas-dock__section-label">Context</div>
            <span className={`canvas-dock-workflow__tone canvas-dock-workflow__tone--${contextTone(summary.contextAuthority)}`}>
          ...
          <div className="canvas-dock-workflow__grid">
            ...
            <div className={`canvas-dock-workflow__row${summary.contextAuthority.blockerCount > 0 ? ' canvas-dock-workflow__row--warn' : ''}`}>
              <span>Blockers</span>
          ...
          <div className="canvas-dock-workflow__pills">
            {summary.contextAuthority.objectLabels.length > 0
              ...
              : <span className="canvas-dock-workflow__pill-empty">no objects</span>}
          </div>
          <div className="canvas-dock-workflow__pills">
            {summary.contextAuthority.ioModes.length > 0
              ? summary.contextAuthority.ioModes.map((label) => <span key={label}>{label}</span>)
              : <span className="canvas-dock-workflow__pill-empty">no io mode</span>}
          </div>
          <div className="canvas-dock-workflow__row">
            <span>Verifiers</span>
          ...
          {summary.contextAuthority.nextActions.length > 0 ? (
            <div className="canvas-dock-workflow__message">
      ...
      <div className="canvas-dock-section-card canvas-dock-section-card--dense">
        <div className="canvas-dock-section-card__header">
          <div className="canvas-dock__section-label">Operating model</div>
          <span className={`canvas-dock-workflow__tone canvas-dock-workflow__tone--${operatingModelTone}`}>
            {operatingModelStatus?.state || 'unchecked'}
          </span>
        </div>
        <div className="canvas-dock-workflow__grid">
          <div className={`canvas-dock-workflow__row${operatingModelStatus?.errorCount ? ' canvas-dock-workflow__row--warn' : ''}`}>
            <span>Builder</span>
        ...
        {operatingModelStatus?.message ? (
          <div className="canvas-dock-workflow__message">{operatingModelStatus.message}</div>
        ) : null}
        {onCheckOperatingModel ? (
          <button
            type="button"
            className="canvas-dock-workflow__action"
      ...
      {operatingModelCompositeStatus ? (
        <div className="canvas-dock-section-card canvas-dock-section-card--dense">
          <div className="canvas-dock-section-card__header">
            <div className="canvas-dock__section-label">Deployability</div>
            <span className={`canvas-dock-workflow__tone canvas-dock-workflow__tone--${compositeModelTone}`}>
          ...
          <div className="canvas-dock-workflow__grid">
            ...
            <div className={`canvas-dock-workflow__row${operatingModelCompositeStatus.blockerCount ? ' canvas-dock-workflow__row--warn' : ''}`}>
              <span>Blockers</span>
          ...
          <div className="canvas-dock-workflow__message">{operatingModelCompositeStatus.message}</div>
        </div>
      ) : null}
    </div>
  );
...
export function CanvasNodeDetail({ node, content, workflowId, onMutate, onCommitAuthorityAction, onClose, selectedEdge, edgeFromLabel, edgeToLabel, onApplyGate, gateItems = [], buildGraph, onUpdateBuildGraph, onCommitGraphAction, contractSuggestionExtras = null, workflowSummary = null, workflowContext = null, operatingModelStatus = null, operatingModelCompositeStatus = null, onCheckOperatingModel }: Props) {
  ...
  useEffect(() => {
    ...
    const qs = triggerSourceRef.trim()
      ...
      : '';
    fetch(`/api/canvas/pickers/payload-fields${qs}`)
      .then(async (response) => {
  ...
  const handleSaveNodePrimitive = useCallback(async () => {
    ...
    try {
      ...
      if (workflowId && node.id && nodePrompt.trim()) {
        recordCanvasHistory(`prompt:${workflowId}:${node.id}`, nodePrompt);
      }
    } catch (e: any) {
  ...
  const handleSaveConditionalEdge = useCallback(async () => {
    ...
    try {
      ...
      if (workflowId && selectedEdge?.id && edgeConditionMode === 'json' && edgeConditionText.trim()) {
        recordCanvasHistory(`condition:${workflowId}:${selectedEdge.id}`, edgeConditionText);
      }
    } catch (e: any) {
  ...
  return (
    <>
      <button className="canvas-dock__close" onClick={onClose} aria-label="Close detail panel">&times;</button>

      {/* Gate config — when an edge is selected */}
      {selectedEdge && onApplyGate ? (
        <>
          <div className="canvas-dock-header">
            <div className="canvas-dock__title">Gate</div>
            <div className="canvas-dock__subtitle">{edgeFromLabel} &rarr; {edgeToLabel}</div>
          </div>
          <div className="canvas-dock-section-card canvas-dock-section-card--dense">
            <div className="canvas-gate-panel">
              <div className="canvas-gate-panel__meta">
                <span className="canvas-surface-badge" title={selectedEdge.gateState === 'empty' ? "No approval required" : undefined}>
                  {selectedGateCatalogModel?.policy.badge || (selectedEdge.gateState === 'empty' ? 'Ungated' : 'Gate')}
                </span>
                {selectedGateCatalogModel && (
                  <span className={`canvas-truth-badge canvas-truth-badge--${selectedGateCatalogModel.truth.category}`}>
                    {selectedGateCatalogModel.truth.badge}
                  </span>
                )}
              </div>
              <div className="canvas-gate-panel__title">
            ...
            {isFailureEdge && (
              <div className="canvas-branch-editor__hint">
            ...
            {isConditionalEdge && (
              <>
                <div className="canvas-branch-editor">
                  <div className="canvas-branch-editor__routes" aria-label="Branch routes">
                    <div className={`canvas-branch-editor__route${buildEdgeRelease?.branch_reason === 'then' ? ' canvas-branch-editor__route--active' : ''}`}>
                      <span className="canvas-branch-editor__route-label">Then</span>
                      <span className="canvas-branch-editor__route-target">{conditionalTargets.thenTarget || edgeToLabel || 'Then path'}</span>
                    </div>
                    <div className={`canvas-branch-editor__route${buildEdgeRelease?.branch_reason === 'else' ? ' canvas-branch-editor__route--active' : ''}`}>
                      <span className="canvas-branch-editor__route-label">Else</span>
                      <span className="canvas-branch-editor__route-target">{conditionalTargets.elseTarget || 'Inverse path'}</span>
                    </div>
                  </div>

                  <div className="canvas-branch-editor__control">
                    <label className="canvas-dock-form__label" htmlFor="canvas-branch-editor-mode">Condition mode</label>
                    <div className="canvas-dock-form__row">
                      <select
                        id="canvas-branch-editor-mode"
                        className="canvas-dock-form__select"
                ...
                <div className="canvas-dock-form__actions">
                  <button
                    className="canvas-dock-form__btn"
                    onClick={handleSaveConditionalEdge}
                    disabled={edgeConditionLoading || !buildGraph || !canCommitGraph}
                  >
                    {edgeConditionLoading ? <><span className="canvas-spinner" /> Saving...</> : 'Save branch condition'}
                  </button>
                </div>
                {edgeConditionError && <div className="canvas-dock-form__error">{edgeConditionError}</div>}
              </>
            )}
            ...
            {selectedEdge.gateState !== 'empty' && (
              <>
                <div className="canvas-dock-form__actions">
                  <CanvasConfirmButton
                    label={gateClearLoading ? <><span className="canvas-spinner" /> Removing...</> : 'Remove gate'}
                ...
                {gateClearError && <div className="canvas-dock-form__error">{gateClearError}</div>}
              </>
            )}
          </div>
        </>
        ...
        <div className="canvas-dock-header">
          <div className="canvas-dock__title">{node ? node.title : 'Detail'}</div>
          {node?.route ? <div className="canvas-dock__subtitle">Route: {node.route}</div> : null}
        </div>
      )}
      ...
      {!node && !selectedEdge ? (
        ...
        <>
          ...
          {contextRibbon && (
            <div className="canvas-detail__ribbon" aria-label="Node context">
              <div className="canvas-detail__ribbon-col canvas-detail__ribbon-col--upstream">
                <div className="canvas-dock__section-label canvas-detail__ribbon-heading">Receives</div>
                {contextRibbon.upstream.length === 0 ? (
                  <div className="canvas-detail__ribbon-empty">origin</div>
                ) : (
                  contextRibbon.upstream.map((n) => (
                    <div key={n.edgeId} className={`canvas-detail__ribbon-item canvas-detail__ribbon-item--${n.family}`}>
                      <span className="canvas-detail__ribbon-dot" aria-hidden="true" />
                      <span className="canvas-detail__ribbon-label">{n.title}</span>
                      <span className="canvas-detail__ribbon-family">{ribbonFamilyLabel(n.family)}</span>
                    </div>
                  ))
                )}
              </div>
              <div className="canvas-detail__ribbon-center">
                <span className="canvas-detail__ribbon-center-dot" aria-hidden="true" />
                <span className="canvas-detail__ribbon-center-label">{node.title || 'this'}</span>
              </div>
              <div className="canvas-detail__ribbon-col canvas-detail__ribbon-col--downstream">
                <div className="canvas-dock__section-label canvas-detail__ribbon-heading">Produces</div>
                {contextRibbon.downstream.length === 0 ? (
                  <div className="canvas-detail__ribbon-empty">terminal</div>
                ) : (
          ...
          {nodeContext && (
            <div className="canvas-dock-section-card canvas-dock-section-card--dense" aria-label="Selected node workflow context">
              <div className="canvas-dock-section-card__header">
                <div className="canvas-dock__section-label">Context</div>
                <span className={`canvas-dock-workflow__tone canvas-dock-workflow__tone--${nodeContext.blockerCount > 0 ? 'blocked' : 'warning'}`}>
                  {nodeContext.pill}
                </span>
              </div>
              <div className="canvas-dock-workflow__grid">
                ...
                <div className={`canvas-dock-workflow__row${nodeContext.blockerCount > 0 ? ' canvas-dock-workflow__row--warn' : ''}`}>
                  <span>Blockers</span>
              ...
              {nodeContext.nextAction ? (
                <div className="canvas-dock-workflow__message">{nodeContext.nextAction}</div>
              ) : null}
            </div>
          )}
          {hasAgentPacket && (
            <div className="canvas-dock-section-card canvas-dock-section-card--dense" aria-label="Agent packet contract">
              <div className="canvas-dock-section-card__header">
                <div className="canvas-dock__section-label">Agent packet</div>
                <div className="canvas-run-contract__badges">
                  <span className="canvas-truth-badge canvas-truth-badge--runtime">One node</span>
                  <span className="canvas-truth-badge canvas-truth-badge--persisted">Field edits</span>
                </div>
              </div>
              <div className="canvas-run-contract__grid">
                {node.taskType && (
                  <div className="canvas-run-contract__row">
                    <span className="canvas-run-contract__label">Task</span>
                    <span className="canvas-run-contract__value">{node.taskType}</span>
                  </div>
                )}
                <CanvasAgentPicker
                  value={node.agent}
                  agents={agentRegistry}
                  onChange={(ref: string | null) => onMutate(`nodes/${node.id}/agent`, { value: ref })}
                />
                {agentToolName && (
                  <div className="canvas-run-contract__row">
                    <span className="canvas-run-contract__label">Tool lane</span>
                    <span className="canvas-run-contract__value">
                ...
                {agentToolTargets.length > 0 && (
                  <div className="canvas-run-contract__row">
                    <span className="canvas-run-contract__label">Fields</span>
                    <span className="canvas-run-contract__value">{agentToolTargets.join(', ')}</span>
                  </div>
                )}
                {nodeCapabilities.length > 0 && (
                  <div className="canvas-run-contract__row">
                    <span className="canvas-run-contract__label">Capabilities</span>
                    <span className="canvas-run-contract__value">{nodeCapabilities.join(', ')}</span>
                  </div>
                )}
                {nodeWriteScope.length > 0 && (
                  <div className="canvas-run-contract__row">
                    <span className="canvas-run-contract__label">Writes</span>
                    <span className="canvas-run-contract__value">{nodeWriteScope.join(', ')}</span>
                  </div>
                )}
                {agentToolFocus && (
                  <div className="canvas-run-contract__row">
                    <span className="canvas-run-contract__label">Focus</span>
                    <span className="canvas-run-contract__value">{agentToolFocus}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          {hasRunContract && (
            <div className="canvas-dock-section-card canvas-dock-section-card--dense" aria-label="Run completion gate">
              <div className="canvas-dock-section-card__header">
                <div className="canvas-dock__section-label">Completion gate</div>
                <div className="canvas-run-contract__badges">
              ...
              <div className="canvas-run-contract__grid">
                {node.taskType && (
                  <div className="canvas-run-contract__row">
                    <span className="canvas-run-contract__label">Task type</span>
                    <span className="canvas-run-contract__value">{node.taskType}</span>
                  </div>
                )}
                {runContractResultKind && (
                  <div className="canvas-run-contract__row">
                    <span className="canvas-run-contract__label">Result</span>
                    <span className="canvas-run-contract__value">{runContractResultKind}</span>
                  </div>
                )}
                {runContractSubmitTools.length > 0 && (
                  <div className="canvas-run-contract__row">
                    <span className="canvas-run-contract__label">Submit with</span>
                    <span className="canvas-run-contract__value">{runContractSubmitTools.join(', ')}</span>
                  </div>
                )}
                {node.outcomeGoal && (
                  <div className="canvas-run-contract__row">
                    <span className="canvas-run-contract__label">Outcome</span>
                    <span className="canvas-run-contract__value">{node.outcomeGoal}</span>
                  </div>
                )}
                {node.description && (
                  <div className="canvas-run-contract__row">
                    <span className="canvas-run-contract__label">Scope</span>
                    <span className="canvas-run-contract__value">{node.description}</span>
                  </div>
                )}
              </div>
              {node.prompt && (
                <details className="canvas-run-contract__prompt">
          ...
          {isTriggerNode && (
            <div className="canvas-dock-section-card">
              <div className="canvas-dock-section-card__header">
                <div className="canvas-dock__section-label">Trigger config</div>
                <div className="canvas-dock__item-desc" style={{ marginTop: 0, fontWeight: 600 }}>
                  {triggerRoute === TRIGGER_SCHEDULE_ROUTE
              ...
              {triggerRoute === TRIGGER_SCHEDULE_ROUTE ? (
                <>
                  <label className="canvas-dock-form__label" htmlFor="canvas-trigger-cron">Schedule</label>
                  <CanvasCronBuilder
                    id="canvas-trigger-cron"
                ...
                <>
                  <label className="canvas-dock-form__label" htmlFor="canvas-trigger-source-ref">
                    Source reference
                  </label>
                  <CanvasPickerInput
                    id="canvas-trigger-source-ref"
                    ...
                    suggestionsUrl={triggerRoute === TRIGGER_WEBHOOK_ROUTE ? '/api/canvas/pickers/webhook-sources' : undefined}
              ...
              <div className="canvas-dock-subsection">
                <div className="canvas-dock__section-label" style={{ marginTop: 0 }}>Event filter</div>
                {triggerNeedsJsonEditor ? (
                  <CanvasJsonEditor
                    id="canvas-trigger-event-filter"
                  ...
                  <>
                    <div className="canvas-trigger-pill-row">
                      {triggerFilterRows.map((row) => (
                        <button
                          key={row.id}
                          type="button"
                          className={`canvas-trigger-pill${activeTriggerFilterId === row.id ? ' canvas-trigger-pill--active' : ''}`}
                          onClick={() => setActiveTriggerFilterId(row.id)}
                        >
                          <span className="canvas-trigger-pill__label">{triggerFilterChipText(row)}</span>
                        </button>
                      ))}
                      <button
                        type="button"
                    ...
                    <div className="canvas-trigger-pill-suggestions">
                      {TRIGGER_FILTER_SUGGESTIONS.map((suggestion) => (
                        <button
                          key={suggestion.label}
                          type="button"
                          className="canvas-trigger-pill-suggestion"
                    ...
                    {activeTriggerFilterRow && (
                      <div className="canvas-trigger-pill-editor">
                        <div className="canvas-dock-form__row" style={{ marginBottom: 4 }}>
                          <CanvasPickerInput
                        ...
                        <div className="canvas-dock-form__actions">
                          <button
                            type="button"
                            className="canvas-dock-form__btn--small"
              ...
              <div className="canvas-dock-form__actions">
                ...
                >
                  {triggerLoading ? <><span className="canvas-spinner" /> Saving...</> : 'Save trigger'}
                </button>
              </div>
              {triggerError && <div className="canvas-dock-form__error">{triggerError}</div>}
            </div>
          )}
          ...
          {showPrimitiveEditor && (
            <div className="canvas-dock-section-card">
              <div className="canvas-dock-section-card__header">
                <div className="canvas-dock__section-label">Step details</div>
              ...
              {!isTriggerNode && (
                <div className="canvas-dock-subsection">
                  <div className="canvas-dock__section-label" style={{ marginTop: 0 }}>Prompt</div>
                  <textarea
                    className="canvas-dock-form__input"
                    value={nodePrompt}
                  ...
                  <CanvasPickerInput
                    ...
                    suggestionsUrl="/api/canvas/pickers/authorities"
              ...
              <div className="canvas-node-advanced">
                <button
                  type="button"
                  className="canvas-node-advanced__toggle"
                  onClick={() => setShowAdvancedContractFields((current) => !current)}
                  aria-expanded={showAdvancedContractFields}
                  aria-controls="canvas-node-advanced-contracts"
                >
                  <span className="canvas-node-advanced__toggle-title">
                    {showAdvancedContractFields ? 'Hide advanced contract fields' : 'Show advanced contract fields'}
                  </span>
                  <span className="canvas-node-advanced__toggle-copy">
                    Required inputs, outputs, and persistence targets stay tucked away unless you need exact control.
                  </span>
                </button>
                {showAdvancedContractFields && (
                  <div id="canvas-node-advanced-contracts" className="canvas-node-advanced__body">
                    <div className="canvas-dock__section-label" style={{ marginTop: 0 }}>Required inputs</div>
                    <CanvasContractStringListField
                      fieldId="canvas-node-required-inputs"
                    ...
                    />
                    <div className="canvas-dock__section-label" style={{ marginTop: 12 }}>Outputs</div>
                    <CanvasContractStringListField
                      fieldId="canvas-node-outputs"
                      ariaLabel="Outputs"
                    ...
                    />
                    <div className="canvas-dock__section-label" style={{ marginTop: 12 }}>Persistence targets</div>
                    <CanvasContractStringListField
                      fieldId="canvas-node-persistence-targets"
              ...
              {isNotificationRoute && (
                <div className="canvas-dock-subsection">
                  <div className="canvas-dock__section-label" style={{ marginTop: 0 }}>Notification properties</div>
                  <input
                    className="canvas-dock-form__input"
                    type="text"
              ...
              {isWebhookRoute && (
                <div className="canvas-dock-subsection">
                  <div className="canvas-dock__section-label" style={{ marginTop: 0 }}>HTTP request properties</div>
                  <div className="canvas-http-request__preset-grid">
                    {HTTP_REQUEST_PRESETS.map((preset) => (
                      <button
                        key={preset.value}
                        type="button"
                        className={`canvas-http-request__preset${httpRequestPreset === preset.value ? ' canvas-http-request__preset--active' : ''}`}
                  ...
                  <div className="canvas-dock__item-desc" style={{ marginBottom: 8 }}>
              ...
              {isWorkflowInvokeRoute && (
                <div className="canvas-dock-subsection">
                  <div className="canvas-dock__section-label" style={{ marginTop: 0 }}>Workflow invoke properties</div>
                  <select
                    className="canvas-dock-form__select"
              ...
              {isIntegrationRoute && !isNotificationRoute && !isWebhookRoute && !isWorkflowInvokeRoute && (
                <div className="canvas-dock-subsection">
                  <div className="canvas-dock__section-label" style={{ marginTop: 0 }}>Integration properties</div>
                  <CanvasJsonEditor
              ...
              <div className="canvas-dock-form__actions">
                ...
                >
                  {nodeSaveLoading ? <><span className="canvas-spinner" /> Saving...</> : 'Save step'}
                </button>
              </div>
              {nodeSaveError && <div className="canvas-dock-form__error">{nodeSaveError}</div>}
            </div>
          )}

          {requiresSavedWorkflow ? (
            <div className="canvas-dock-section-card">
              <div className="canvas-dock__section-label">Authority-backed tools</div>
              <div className="canvas-dock__item" style={{ cursor: 'default' }}>
                <div className="canvas-dock__item-title">Save draft to unlock imports and attachments</div>
              </div>
            </div>
          ) : (
            <>
              <div className="canvas-dock-section-card">
                ...
                {(content?.contextAttachments || []).map(a => (
                  <div
                    key={a.attachment_id}
                    className="canvas-dock__item"
                    draggable
                    onDragStart={e => {
                      e.dataTransfer.setData('canvas/dock', 'context');
                      e.dataTransfer.setData('text/plain', a.label || a.authority_ref || '');
                      e.dataTransfer.effectAllowed = 'link';
                    }}
                  >
                    <div className="canvas-dock__item-title">{a.label || a.authority_ref || 'Reference'}</div>
                    <div className="canvas-dock__item-desc">{a.authority_kind} / {a.role || 'input'}</div>
                  </div>
                ))}
                {!(content?.contextAttachments || []).length && (
                  <div className="canvas-dock__empty">No attachments yet.</div>
                )}
              </div>
              ...
              <div className="canvas-dock-section-card">
                ...
                {(content?.imports || []).map(s => (
                  <div key={s.snapshot_id} className="canvas-dock__item">
                    <div className="canvas-dock__item-title">{s.source_locator || s.snapshot_id}</div>
                    <div className="canvas-dock__item-desc">
                ...
                {!(content?.imports || []).length && (
                  <div className="canvas-dock__empty">No imports staged.</div>
                )}
              </div>
              ...
              <div className="canvas-dock-section-card">
                ...
                {!bindings.length && (
                  <div className="canvas-dock__empty">No bindings.</div>
                )}
              </div>
              ...
              <div className="canvas-dock-section-card">
                ...
                {objectTypesLoading ? (
                  <div className="canvas-dock__empty"><span className="canvas-spinner" /> Loading...</div>
                ) : objectTypes.length === 0 ? (
                  <div className="canvas-dock__empty">No object types registered.</div>
                ) : (
                  <div className="canvas-dock__catalog-grid">
                    {objectTypes
                      ...
                      .map(ot => (
                        <div
                          key={ot.type_id}
                          className="canvas-dock__catalog-item"
                          draggable
                          onDragStart={e => {
                            e.dataTransfer.setData('canvas/object-type-id', ot.type_id);
                            e.dataTransfer.setData('canvas/object-type-label', ot.name);
              ...
              <div className="canvas-dock-section-card">
                <div className="canvas-dock__section-label">Attach reference</div>

                <div className="canvas-dock-form__row">
                  <select className="canvas-dock-form__select" value={attachKind} onChange={e => setAttachKind(e.target.value)}>
                    <option value="reference">Reference</option>
                    <option value="object_type">Object Type</option>
                    <option value="object">Object</option>
                  </select>
                  <select className="canvas-dock-form__select" value={attachRole} onChange={e => setAttachRole(e.target.value)}>
                ...
                <input className="canvas-dock-form__input" type="text" value={attachRef} onChange={e => setAttachRef(e.target.value)} placeholder="Authority ref (slug or ID)" />
                <input className="canvas-dock-form__input" type="text" value={attachLabel} onChange={e => setAttachLabel(e.target.value)} placeholder="Label (optional)" />

                <label className="canvas-dock-form__checkbox">
                  <input type="checkbox" checked={attachPromote} onChange={e => setAttachPromote(e.target.checked)} />
                  Promote to state node
                </label>

                <div className="canvas-dock-form__actions">
                  <button className="canvas-dock-form__btn" onClick={handleAttach} disabled={attachLoading || !attachRef.trim()}>
                    {attachLoading ? <><span className="canvas-spinner" /> Attaching...</> : 'Attach'}
                  </button>
                </div>
                {attachError && <div className="canvas-dock-form__error">{attachError}</div>}
              </div>

              <div className="canvas-dock-section-card">
                <div className="canvas-dock__section-label">Stage import</div>

                <input className="canvas-dock-form__input" type="text" value={importLocator} onChange={e => setImportLocator(e.target.value)} placeholder="URL or source locator" />
                <input className="canvas-dock-form__input" type="text" value={importLabel} onChange={e => setImportLabel(e.target.value)} placeholder="Label (optional)" />

                <div className="canvas-dock-form__actions">
                  <button className="canvas-dock-form__btn" onClick={handleStageImport} disabled={importLoading || !importLocator.trim()}>
                    {importLoading ? <><span className="canvas-spinner" /> Staging...</> : 'Stage'}
                  </button>
                  <button className="canvas-dock-form__btn" onClick={handleMaterialize} disabled={importLoading || !importLocator.trim()}>
                    Materialize here
                  </button>
                </div>
                {importError && <div className="canvas-dock-form__error">{importError}</div>}
              </div>
            </>
          )}
        </>
...
function BindingCard({
  ...
  const stateColor = isAccepted ? 'var(--canvas-status-ok)' : isRejected ? 'var(--canvas-error)' : 'var(--canvas-fg-muted)';

  return (
    <div className={`canvas-dock__item${isRejected ? ' canvas-dock__item--muted' : ''}`}>
      <div className="canvas-dock__item-title">
          ...
          {binding.accepted_target.enrichment ? (
            <>
              ...
              {binding.accepted_target.enrichment.auth_status && (
                <span style={{ marginLeft: 6, fontSize: 10, color: binding.accepted_target.enrichment.auth_status === 'connected' ? 'var(--canvas-status-ok)' : 'var(--canvas-fg-muted)' }}>
                  ({binding.accepted_target.enrichment.auth_status})
                </span>
              )}
              {binding.accepted_target.enrichment.description && (
                <div style={{ fontSize: 11, color: 'var(--canvas-fg-muted)', marginTop: 2 }}>
                  {binding.accepted_target.enrichment.description}
                </div>
              )}
            </>
      ...
      {!isAccepted && !isRejected && binding.candidate_targets?.length ? (
        <div className="canvas-dock-subsection" style={{ marginTop: 8 }}>
          <div className="canvas-dock__section-label">Pick a target:</div>
          {binding.candidate_targets.map((target, i) => (
            ...
            >
              ...
              {target.enrichment?.auth_status ? (
                <span className="canvas-dock__item-desc" style={{ marginLeft: 6, color: target.enrichment.auth_status === 'connected' ? 'var(--canvas-status-ok)' : 'var(--canvas-fg-muted)' }}>
                  ({target.enrichment.auth_status})
                </span>
              ) : target.kind ? (
                <span className="canvas-dock__item-desc" style={{ marginLeft: 6 }}>({target.kind})</span>
              ) : null}
            </button>
          ))}
        </div>
      ...
      {!isAccepted && !isRejected && (
        <div className="canvas-dock-form__actions">
          {!showReplace ? (
            <button className="canvas-dock-form__btn--small" onClick={() => setShowReplace(true)}>
              Custom target
            </button>
          ) : (
            <>
              <input className="canvas-dock-form__input" style={{ marginBottom: 0, flex: 1 }} type="text" value={replaceRef} onChange={e => setReplaceRef(e.target.value)} placeholder="Target ref" />
              <button className="canvas-dock-form__btn" onClick={handleReplace} disabled={loading || !replaceRef.trim()}>
                {loading ? '...' : 'Use'}
              </button>
              <button className="canvas-dock-form__btn--small" onClick={() => setShowReplace(false)}>Cancel</button>
            </>
          )}
          <button className="canvas-dock-form__btn--small" style={{ color: 'var(--canvas-error)', borderColor: 'var(--canvas-error)' }} onClick={handleReject} disabled={loading}>
            Reject
          </button>
        </div>
      )}
