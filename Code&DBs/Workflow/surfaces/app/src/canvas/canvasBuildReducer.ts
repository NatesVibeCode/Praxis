 ...
 */
export type CanvasViewMode = 'build' | 'run';

export interface CanvasBuildState {
  ...
  /** How Canvas entered run-view: via URL (external) or DISPATCH_SUCCESS. */
...
export const initialCanvasBuildState: CanvasBuildState = {
  viewMode: 'build',
  activeNodeId: null,
  selectedNodeId: null,
  selectedEdgeId: null,
...
export function canvasBuildReducer(state: CanvasBuildState, action: CanvasBuildAction): CanvasBuildState {
  switch (action.type) {
    case 'SELECT_NODE':
      return {
        ...state,
    ...
    case 'DISPATCH_SUCCESS':
      ...
      return canvasBuildReducer(state, { type: 'ENTER_RUN_VIEW', runId: action.runId, source: 'dispatch' });
    case 'ENTER_RUN_VIEW':
      return {
        ...state,
        viewMode: 'run',
    ...
    case 'RESET':
      return initialCanvasBuildState;
    default:
      return state;
  }
}
