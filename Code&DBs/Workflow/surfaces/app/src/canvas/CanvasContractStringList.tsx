...
export interface CanvasContractStringListFieldProps {
  fieldId: string;
  ariaLabel: string;
  hint?: string;
...
export function CanvasContractStringListField({
  fieldId,
  ariaLabel,
  hint,
  ...
  rows,
  onChange,
}: CanvasContractStringListFieldProps) {
  ...
  return (
    <div className="canvas-contract-string-list" aria-label={ariaLabel}>
      <div className="canvas-trigger-pill-row" role="list">
        {rows.map((row) => (
          <button
            key={row.id}
            type="button"
            role="listitem"
            className={`canvas-trigger-pill${activeId === row.id ? ' canvas-trigger-pill--active' : ''}`}
            onClick={() => setActiveId(row.id)}
            aria-pressed={activeId === row.id}
            aria-label={`${ariaLabel} field: ${chipLabel(row)}`}
          >
            <span className="canvas-trigger-pill__label">{chipLabel(row)}</span>
          </button>
        ))}
        <button
          type="button"
      ...
      {activeRow && (
        <div className="canvas-trigger-pill-editor">
          <label className="canvas-dock-form__label" htmlFor={inputId}>
            Edit selected field
          </label>
          <input
            id={inputId}
          ...
          {filteredContractSuggestions.length > 0 ? (
            <>
              <div className="canvas-contract-suggestions-label" id={`${fieldId}-suggest-label`}>
                Suggested from this graph and object types
              </div>
              <div
                className="canvas-contract-suggestions"
                id={`${fieldId}-suggest`}
                role="listbox"
              ...
              >
                {filteredContractSuggestions.map((s) => (
                  <button
                    ...
                    title={s.detail}
                  >
                    <span className="canvas-contract-suggestion__value">{s.value}</span>
                    {s.detail ? (
                      <span className="canvas-contract-suggestion__detail">{s.detail}</span>
                    ) : null}
                  </button>
                ))}
              </div>
