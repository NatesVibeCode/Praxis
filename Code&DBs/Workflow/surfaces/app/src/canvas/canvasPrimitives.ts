...
export interface CanvasPrimitiveSpec {
  route: string;
...
const CANVAS_PRIMITIVES: Record<string, CanvasPrimitiveSpec> = {
  'auto/research': {
    route: 'auto/research',
    surface: {
      tier: 'primary',
...
export function getCanvasPrimitive(route?: string | null): CanvasPrimitiveSpec | null {
  if (!route) return null;
  return CANVAS_PRIMITIVES[route] || null;
}

export function scaffoldCanvasPrimitiveNode(
  node: BuildNode,
  options: {
    actionValue: string;
    title: string;
  ...
  const primitive = getCanvasPrimitive(options.actionValue);
