...
import { presentBuild } from './canvasBuildPresenter';
import type { CanvasBuildViewModel, OrbitNode, OrbitEdge, RunJobStatus } from './canvasBuildPresenter';
import { presentRun } from './canvasRunPresenter';
...
import { CanvasNodeDetail, type AuthorityActionMeta, type WorkflowInspectorSummary } from './CanvasNodeDetail';
import {
...
import { CanvasBindingReviewQueue, reviewReadinessCount } from './CanvasBindingReviewQueue';
import { CanvasRunPanel } from './CanvasRunPanel';
import { CanvasRunOverlay } from './CanvasRunOverlay';
import { CanvasDragGhost } from './CanvasDragGhost';
import { CanvasOutcomeContract } from './CanvasOutcomeContract';
import { CanvasEdges, edgePresentation, getEdgeGeometry } from './CanvasEdges';
...
import { scaffoldCanvasPrimitiveNode } from './canvasPrimitives';
import {
  buildAuthorityCompileProse,
  summarizeComposeAuthority,
  type CanvasComposeAuthoritySummary,
} from './canvasComposeAuthority';
import {
...
import { setCanvasChatContext, clearCanvasChatContext, publishCanvasChatHandoff } from './canvasChatContext';
import {
  getCanvasAppendPosition,
  getCanvasCanvasDimensions,
  getCanvasNodeAnchorRect,
  getCanvasNodeCanvasPosition,
  CANVAS_LAYOUT,
  CANVAS_LAYOUT_CSS_VARS,
} from './canvasLayout';
...
import './canvas-build.css';

interface Props {
  workflowId: string | null;
  /**
   * When set, Canvas renders a run-view over its canvas for this run_id.
...
type CanvasGlowProfile = 'soft' | 'strict';

function readCanvasGlowProfile(): CanvasGlowProfile {
  if (typeof window === 'undefined') return 'soft';
  const storage = window.localStorage;
  if (!storage || typeof storage.getItem !== 'function') return 'soft';
  try {
    return storage.getItem('canvas-glow-profile') === 'strict' ? 'strict' : 'soft';
  } catch {
    return 'soft';
  }
}
...
function buildMaterializeFallbackPrompt(params: {
  ...
  return [
    ...
    'If a workflow id exists, first call canvas_get_build, the legacy Workflow graph read tool, before making any claim about the graph.',
    'If you can repair the draft, use canvas_mutate_field, the legacy Workflow graph mutation tool, one field of one node at a time.',
...
function agentToolPlanSummary(viewModel: CanvasBuildViewModel): string {
...
function branchReviewSummary(viewModel: CanvasBuildViewModel): string {
...
function workflowInspectorSummary(
  ...
  viewModel: CanvasBuildViewModel,
...
function compactCanvasVisibleNode(node: BuildNode): Record<string, unknown> {
...
function compactCanvasVisibleEdge(edge: BuildEdge): Record<string, unknown> {
...
function canvasVisibleSnapshot(
  payload: BuildPayload | null,
  ...
  const snapshot: Record<string, unknown> = {
    ...
    nodes: nodes.slice(0, 16).map(compactCanvasVisibleNode),
    edges: edges.slice(0, 24).map(compactCanvasVisibleEdge),
  };
  if (selectedNode) {
  ...
  if (selectedEdge) {
    snapshot.selected_edge = compactCanvasVisibleEdge(selectedEdge);
  }
...
function clampScale(value: number): number {
  ...
  return Math.max(CANVAS_MIN_SCALE, Math.min(CANVAS_MAX_SCALE, value));
}
...
function nodeCardClass(node: OrbitNode, isSelected: boolean, isDragOver: boolean): string {
  const classes = [
    'canvas-graph-node',
    `canvas-graph-node--kind-${node.kind}`,
    `canvas-graph-node--shape-${node.shape}`,
    `canvas-graph-node--state-${node.ringState}`,
    isSelected ? 'canvas-graph-node--selected' : '',
    isDragOver ? 'canvas-graph-node--drag-over' : '',
    node.multiplicity ? `canvas-graph-node--stack canvas-graph-node--stack-${node.multiplicity.kind}` : '',
  ];
...
const INITIAL_COMPOSE_AUTHORITY: CanvasComposeAuthoritySummary = {
  status: 'loading',
...
function shouldKeepEdgeMenusOpen(target: EventTarget | null): boolean {
  ...
  return Boolean(
    target.closest('.canvas-graph-gate')
    || target.closest('.canvas-dock-overlay')
    || target.closest('.canvas-dock-side')
...
export function CanvasBuildPage({ workflowId, runId, onBack, onWorkflowCreated, onEditWorkflow, onViewRun, onDraftStateChange, onMaterializeHandoff, initialMode }: Props) {
  const { payload, loading, error, mutate, reload, setPayload } = useBuildPayload(workflowId);
  const [state, dispatch] = useReducer(canvasBuildReducer, {
    ...initialCanvasBuildState,
    emptyMode: initialMode ?? initialCanvasBuildState.emptyMode,
  });
  ...
  const [canvasGlowProfile, setCanvasGlowProfile] = useState<CanvasGlowProfile>(readCanvasGlowProfile);
  const [mutationError, setMutationError] = useState<string | null>(null);
  const [composeAuthority, setComposeAuthority] = useState<CanvasComposeAuthoritySummary>(INITIAL_COMPOSE_AUTHORITY);
  ...
  const visibleCanvasSnapshot = useMemo(
    () => canvasVisibleSnapshot(payload, state.selectedNodeId, state.selectedEdgeId, pendingReviewCount),
  ...
  const handleCheckClientOperatingModel = useCallback(async () => {
    ...
    try {
      const status = await fetchWorkflowBuilderValidationStatus(
        ...
        catalog,
        {
          scopeRef: persistedWorkflowId || workflowId || 'canvas.workflow_builder',
        },
      );
  ...
  // The chat orchestrator threads this down to the canvas_* tools so they
  ...
  useEffect(() => {
    ...
    const visibleGraphSummary = visibleCanvasSnapshot
      ? {
          source: 'visible_ui',
          node_count: visibleCanvasSnapshot.node_count,
          edge_count: visibleCanvasSnapshot.edge_count,
          review_count: visibleCanvasSnapshot.review_count,
        }
      : null;
    setCanvasChatContext({
      ...
      hint:
        state.viewMode === 'run'
          ? 'User is viewing a run. canvas_get_build still works; mutations are typically not appropriate while observing a live run.'
          : 'User is authoring a workflow graph. Default-target this workflow_id when the user does not name another. Compare canvas_get_build against visible_ui_snapshot before claiming the graph is empty.',
      ...
      visible_ui_snapshot: visibleCanvasSnapshot,
    });
  }, [
    persistedWorkflowId,
    payload?.workflow,
    ...
    state.viewMode,
    visibleCanvasSnapshot,
  ]);
  // Clear context on unmount so chat opened from elsewhere isn't haunted by
  // a stale Canvas stanza pointing at a workflow the user has navigated away from.
  useEffect(() => {
    return () => {
      clearCanvasChatContext();
    };
  ...
  useEffect(() => {
    const syncProfile = () => setCanvasGlowProfile(readCanvasGlowProfile());
    const onStorage = (event: StorageEvent) => {
      if (event.key === null || event.key === 'canvas-glow-profile') {
        syncProfile();
      }
    };
    syncProfile();
  ...
  const canvasUndoScope = workflowId ? `canvas:${workflowId}` : 'canvas:draft';

  useEffect(() => registerUiActionUndoExecutor('canvas.payload.restore', (descriptor) => {
    if (descriptor.scope !== canvasUndoScope) return false;
    setPayload(descriptor.payload as BuildPayload | null);
    return true;
  }), [canvasUndoScope, setPayload]);
  ...
  const commitCanvasGraphAction = useCallback(async (details: {
    label: string;
    reason: string;
    outcome: string;
    ...
    return entry;
  }, [applyGraphPayload, buildDraftGraphUndoDescriptor, buildMutationUndoDescriptor, handleUndoAction, canvasUndoScope, payload, runMutation, show, workflowId]);

  const commitCanvasAuthorityAction = useCallback(async (details: {
    subpath: string;
  ...
  }, [buildMutationUndoDescriptor, handleUndoAction, canvasUndoScope, runMutation, show]);
  ...
  const handleNodeAction = useCallback(async (nodeId: string, actionValue: string) => {
    ...
    try {
      ...
      const scaffold = scaffoldCanvasPrimitiveNode(nodes[idx], { actionValue, title, summary });
  ...
  }, [catalog, commitCanvasGraphAction, payload, state.activeNodeId, state.openDock]);
  ...
  }, [commitCanvasGraphAction, payload]);
  ...
  }, [catalog, commitCanvasGraphAction, handleCreateBranch, payload]);

  const handleCompile = useCallback(async () => {
    if (!compileSource.trim()) return;
    dispatch({ type: 'COMPILE_START' });
    const handoffBaseId = `canvas-materialize-${Date.now()}`;
    ...
    try {
      ...
      const handoffToCanvasFallback = (params: {
        workflowId?: string | null;
        reason: string;
        receiptId?: string | null;
        correlationId?: string | null;
      ...
      if (wfId) {
        setCanvasChatContext({
          ...
          selected_edge_id: null,
          view_mode: 'build',
          hint: 'Materialize completed for this workflow. Review with canvas_get_build before making edits.',
          ...
          return {
            ...
            meta: item.status === 'coming_soon'
              ...
              : item.source === 'integration' && item.connectionStatus && item.connectionStatus !== 'connected'
                ...
                : truth.badge,
            icon: <CanvasGlyph type={item.icon} size={16} color={item.status === 'ready' ? 'currentColor' : 'var(--canvas-status-idle)'} />,
  ...
  const selectedNodeAnchorRect = useMemo(() => {
    ...
    return getCanvasNodeAnchorRect(centerRef.current.getBoundingClientRect(), selectedNode);
  ...
  }, [commitCanvasGraphAction, payload, state.activeNodeId, state.openDock, state.selectedEdgeId, state.selectedNodeId, state.viewMode]);
  ...
  const drag = useCanvasDrag((dragPayload, target) => {
  ...
  const translateX = useMemo(() => {
    ...
    const activePos = idx * CANVAS_LAYOUT.nodeSpacing + CANVAS_LAYOUT.nodeSize / 2;
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (event: MouseEvent) => {
      ...
      if (target instanceof Element && target.closest('.canvas-node-branch-pod')) return;
      setBranchPickerNodeId(null);
    };
    document.addEventListener('mousedown', handlePointerDown);
  ...
  return (
    <div
      className={`canvas-page${state.viewMode === 'run' ? ' canvas-page--run-view' : ''}`}
      style={CANVAS_LAYOUT_CSS_VARS}
      data-canvas-glow-profile={canvasGlowProfile}
    >
      {mutationError && (
        <div className="canvas-error-toast" role="alert" aria-live="polite">
          {mutationError}
      ...
      <div className="canvas-body">
        {/* Middle row */}
        <div className={middleClassName} data-testid="canvas-middle">
          {/* Left dock: all review/workflow controls. Chat owns the right lane. */}
          <div className={`canvas-dock-overlay canvas-dock-overlay--left${leftControlOpen ? ' canvas-dock-overlay--open' : ''}`}>
            {state.reviewQueueOpen ? (
              <CanvasBindingReviewQueue
                payload={payload}
                onCommitAuthorityAction={async (subpath, body, meta) => {
                  await commitCanvasAuthorityAction({
                    subpath,
                    body,
                    ...meta,
                  });
          ...
          {/* Center */}
          <div className={`canvas-center${!hasNodes ? ' canvas-center--empty' : ''}`} ref={centerRef}>
            {hasNodes && state.viewMode !== 'run' && (
              <>
                <div className="canvas-center__dock-actions" aria-label="Workspace panels">
                  <div className="canvas-center__dock-group">
            ...
            {!hasNodes ? (
              <div className="canvas-start">
                ...
                <div
                  ...
                  className={`canvas-nucleus${state.emptyMode === 'choice' ? ' canvas-nucleus--interactive' : ''}`}
                  onClick={state.emptyMode === 'choice' ? () => dispatch({ type: 'EMPTY_OPEN_SELECTION' }) : undefined}
                >
                  <div className={`canvas-nucleus__ring${state.selectedTrigger ? ' canvas-nucleus__ring--decided' : ''}`}>
                  </div>
                </div>

                {compiling && (
                  <div className="canvas-materialize-wait" role="status" aria-live="polite">
                ...
                {state.compileError && !showComposePanel && (
                  <div className="canvas-compose__error" role="alert">
                    {state.compileError}
                ...
                {showComposePanel && (
                  <div className="canvas-compose canvas-compose--intro" style={{ marginTop: 32 }}>
                    <div className="canvas-compose__title">Describe the workflow</div>
                    <div className="canvas-compose__hint">
                    ...
                    {(compilePreviewLoading || compilePreviewError || compilePreview) && (
                      <div className="canvas-compose-preview" aria-live="polite">
                        {compilePreviewLoading && (
                          <div className="canvas-compose-preview__status">Reading scope...</div>
                        )}
                        {!compilePreviewLoading && compilePreviewError && (
                          <div className="canvas-compose-preview__status canvas-compose-preview__status--error">
                            {compilePreviewError}
                          </div>
                        )}
                        {!compilePreviewLoading && !compilePreviewError && compilePreview && (
                          <>
                            {compilePreviewLabels.matched.length > 0 && (
                              <div className="canvas-compose-preview__row">
                                <div className="canvas-compose-preview__label">Matched</div>
                                <div className="canvas-compose-preview__chips">
                                  {compilePreviewLabels.matched.map((label, index) => (
                                    <span className="canvas-compose-preview__chip" key={`matched-${index}-${label}`}>{label}</span>
                                  ))}
                                </div>
                              </div>
                            )}
                            {compilePreviewLabels.suggested.length > 0 && (
                              <div className="canvas-compose-preview__row">
                                <div className="canvas-compose-preview__label">Suggested</div>
                                <div className="canvas-compose-preview__chips">
                                  {compilePreviewLabels.suggested.map((label, index) => (
                                    <span className="canvas-compose-preview__chip" key={`suggested-${index}-${label}`}>{label}</span>
                                  ))}
                                </div>
                              </div>
                            )}
                            {compilePreviewLabels.gaps.length > 0 && (
                              <div className="canvas-compose-preview__row">
                                <div className="canvas-compose-preview__label">Needs</div>
                                <div className="canvas-compose-preview__chips">
                                  {compilePreviewLabels.gaps.map((label, index) => (
                                    <span className="canvas-compose-preview__chip canvas-compose-preview__chip--gap" key={`gap-${index}-${label}`}>{label}</span>
                                  ))}
                                </div>
                              </div>
                            )}
                    ...
                    <div className="canvas-compose__actions prx-button-row">
                      <Button
                        tone="primary"
                        size="lg"
                    ...
                    {compiling && (
                      <div className="canvas-compose__progress" aria-live="polite" style={{
                        marginTop: 16,
                        padding: '12px 16px',
                        border: '1px solid var(--canvas-border, rgba(255,255,255,0.15))',
                        borderRadius: 6,
                        fontSize: 12,
                        color: 'var(--fg3)',
                      }}>
                    ...
                    <div className="canvas-compose__shortcut">Press Ctrl/Cmd+Enter to compose.</div>
                  </div>
                )}
              </div>
            ) : (
              <div
                ...
                style={{
                  position: 'relative',
                  ...getCanvasCanvasDimensions(viewModel.layout),
                  margin: '0 auto',
                  minHeight: CANVAS_LAYOUT.minGraphHeight,
                ...
                <div style={{ position: 'absolute', top: 0, left: 0 }}>
                  {edgeControls.map((control) => {
                    ...
                    return (
                      <div
                        key={control.edge.id}
                        className={`canvas-graph-gate canvas-graph-gate--${presentation.tone}${isSelected ? ' canvas-graph-gate--selected' : ''}${isDragOver ? ' canvas-graph-gate--drag-over' : ''}${control.edge.gateFamily ? ` canvas-graph-gate--family-${control.edge.gateFamily}` : ''}`}
                        style={{
                      ...
                      >
                          <button
                            type="button"
                            className="canvas-graph-gate__trigger"
                            onClick={() => handleSelectEdge(control.edge.id)}
                            aria-label={gateAriaLabel}
                          >
                          <span
                            className={`canvas-graph-gate__icon${isEmpty ? ' canvas-graph-gate__icon--plus' : ''}`}
                            aria-hidden="true"
                          >
                            {isEmpty ? '+' : <CanvasGlyph type={presentation.glyph} size={12} color="currentColor" />}
                          </span>
                          {!isEmpty && <span className="canvas-graph-gate__trigger-label">{presentation.shortLabel}</span>}
                        </button>

                        {isSelected && (
                          <div className="canvas-graph-gate__card">
                            <div className="canvas-graph-gate__meta">
                              <span className={`canvas-surface-badge canvas-surface-badge--${presentation.tone}`}>
                                {presentation.stateLabel}
                              </span>
                            </div>
                            <div className="canvas-graph-gate__route-label">{presentation.shortLabel}</div>
                            <div className="canvas-graph-gate__title">{title}</div>
                            <div className="canvas-graph-gate__path">
                              {control.fromLabel}{' -> '}{control.toLabel}
                            </div>

                            <button
                              type="button"
                              className="canvas-graph-gate__detail-button"
                  ...
                  {viewModel.nodes.map((node) => {
                    ...
                    const nodeStyle = {
                      ...position,
                      '--canvas-node-instance-width': `${node.width}px`,
                      '--canvas-node-instance-height': `${node.height}px`,
                      opacity: nodeOpacity,
                      transition: 'opacity 240ms ease',
                    ...
                    return (
                      ...
                      >
                        <span className="canvas-graph-node__index" aria-hidden="true">
                          <span className="canvas-graph-node__status-dot" />
                          <span className="canvas-graph-node__index-number">{stepIndex}</span>
                        </span>
                        <span className="canvas-graph-node__body">
                          <span className="canvas-graph-node__title">{displayTitle}</span>
                          {decisionAgentLabel && (
                            <span className="canvas-graph-node__decision-agent" aria-hidden="true">
                              <span>{decisionAgentLabel}</span>
                              <span className="canvas-graph-node__dots" aria-hidden="true">
                                <span>.</span><span>.</span><span>.</span>
                              </span>
                            </span>
                          )}
                          <span className="canvas-graph-node__terminal" aria-hidden="true">
                            <span className="canvas-graph-node__terminal-label">tool</span>
                            <span className="canvas-graph-node__terminal-value">{toolSummary}</span>
                            <span className="canvas-graph-node__terminal-label">state</span>
                            <span className="canvas-graph-node__terminal-value canvas-graph-node__terminal-value--cadence">
                              {statusLabel}
                              <span className="canvas-graph-node__dots" aria-hidden="true">
                                <span>.</span><span>.</span><span>.</span>
                              </span>
                        ...
                        {multiplicityAttr && (
                          <span className="canvas-graph-node__count-pill" aria-label={`${multiplicityAttr} count`}>
                  ...
                  */}
                    ...
                    .map((node) => {
                      ...
                      return (
                        <div
                          ...
                          className={`canvas-node-branch-pod${open ? ' canvas-node-branch-pod--open' : ''}`}
                          style={{ left: podLeft, top: podTop }}
                          data-keep-edge-menu-open="true"
                        >
                          <button
                            type="button"
                            className="canvas-node-branch-pod__trigger"
                          ...
                          {open && (
                            <div className="canvas-node-branch-pod__menu" role="menu">
                              <div className="canvas-node-branch-pod__title">Branch from {node.title}</div>
                              {[
                                ...
                                <button
                                  ...
                                  type="button"
                                  role="menuitem"
                                  className={`canvas-node-branch-pod__option canvas-node-branch-pod__option--${opt.family}`}
                                  onClick={(e) => {
                                    e.stopPropagation();
                                    void handleCreateSiblingBranch(node.id, opt.family);
                                  }}
                                >
                                  <span className="canvas-node-branch-pod__option-label">{opt.label}</span>
                                  <span className="canvas-node-branch-pod__option-hint">{opt.hint}</span>
                                </button>
                              ))}
                            </div>
                          )}
                  ...
                  {state.viewMode !== 'run' && (
                    <div
                      className={`canvas-graph-append${previewTargetId === '__append__' ? ' canvas-graph-append--drag-over' : ''}`}
                      style={appendPosition}
                      data-drop-append="true"
                      onClick={() => appendNode()}
                    >
                      <span className="canvas-graph-append__plus" aria-hidden="true">
                        +
                      </span>
                    </div>
                  )}
                </div>
                <CanvasDragGhost drag={drag.drag} />
              </div>
            )}
        ...
        <div className={`canvas-dock-bottom${state.runViewOpen ? ' canvas-dock-bottom--open' : ' canvas-dock-bottom--closed'}`}>
      ...
      {state.emptyMode === 'trigger-picker' && (
        <MenuPanel
          ...
          width={CANVAS_LAYOUT.triggerMenuWidth}
        />
      )}
