...
interface MoonPickerInputProps {
  id?: string;
  value: string;
...
export function invalidateMoonPickerCache(url?: string) {
  if (url) {
...
function nextDatalistId(): string {
  _datalistSerial += 1;
  return `moon-picker-dl-${Date.now()}-${_datalistSerial}`;
}

export function MoonPickerInput({
  id,
  value,
  onChange,
  placeholder,
  ...
  ariaLabel,
  disabled,
  inputClassName = 'moon-dock-form__input',
  style,
  type = 'text',
}: MoonPickerInputProps) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 2, ...style }}>
      ...
      {hint && <div className="moon-dock__item-desc" style={{ marginTop: 0 }}>{hint}</div>}
    </div>
  );
}
...
interface MoonCronBuilderProps {
  value: string;
  onChange: (next: string) => void;
  id?: string;
}

export function MoonCronBuilder({ value, onChange, id }: MoonCronBuilderProps) {
  ...
  return (
    <div className="moon-cron-builder" style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
      <div className="moon-http-request__preset-grid" role="radiogroup" aria-label="Schedule preset">
        {CRON_PRESETS.map((preset) => {
          ...
          return (
            <button
              ...
              className={`moon-http-request__preset${isActive ? ' moon-http-request__preset--active' : ''}`}
              onClick={() => {
                setCustom(false);
        ...
        <button
          ...
          role="radio"
          aria-checked={custom}
          className={`moon-http-request__preset${custom ? ' moon-http-request__preset--active' : ''}`}
          onClick={() => setCustom(true)}
        >
          <span className="moon-http-request__preset-title">Custom</span>
          <span className="moon-http-request__preset-desc">Write a cron expression</span>
        </button>
      </div>
      {custom && (
        <input
      ...
      <div className="moon-dock__item-desc" style={{ marginTop: 0 }}>
        {preview && preview.length > 0 ? (
          ...
          <span style={{ color: 'var(--moon-warning, #c07)' }}>
...
interface MoonLocalHistoryRailProps {
  scopeKey: string;
  currentValue: string;
...
export function recordMoonHistory(scopeKey: string, value: string, limit = 10): void {
  if (!scopeKey) return;
...
export function MoonLocalHistoryRail({
  scopeKey,
  currentValue,
  onRestore,
  limit = 10,
  label = 'Recent saves',
}: MoonLocalHistoryRailProps) {
  ...
  return (
    <div className="moon-history-rail" style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
      <div className="moon-dock__item-desc" style={{ marginTop: 0, fontWeight: 600 }}>{label}</div>
...
export interface MoonJsonEditorTemplate {
  label: string;
...
interface MoonJsonEditorProps {
  ...
  minHeight?: number;
  ariaLabel?: string;
  templates?: MoonJsonEditorTemplate[];
...
export function MoonJsonEditor({
  value,
  onChange,
  id,
  placeholder,
  ...
  const applyTemplate = (tpl: MoonJsonEditorTemplate) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
      <textarea
        ...
        placeholder={placeholder}
        rows={rows}
        style={{ minHeight, resize: 'vertical', fontFamily: 'var(--moon-mono, ui-monospace, SFMono-Regular, Menlo, monospace)', fontSize: 12 }}
        aria-label={ariaLabel}
        spellCheck={false}
        disabled={disabled}
      />
...
// already happens on every keystroke elsewhere in Moon, so this pattern only
// guards the handful of true destroy-data actions (delete, clear, remove).

interface MoonConfirmButtonProps {
...
export function MoonConfirmButton({
  onConfirm,
  label,
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
      ...
      {primed && primedHint && (
        <span className="moon-dock__item-desc" style={{ fontSize: 11, opacity: 0.75 }}>{primedHint}</span>
      )}
    </span>
  );
}
