...
vi.mock('../shared/buildController', () => ({
  materializePlan: moonBuildPageMocks.materializePlan,
  previewCompile: moonBuildPageMocks.previewCompile,
}));
...
describe('MoonBuildPage', () => {
  beforeEach(() => {
    ...
    moonBuildPageMocks.previewCompile.mockResolvedValue({
      kind: 'compile_preview',
      cqrs_role: 'query',
      scope_packet: { suggested_steps: [] },
    });
    moonBuildPageMocks.materializePlan.mockResolvedValue({
      definition: {},
      build_graph: {
  ...
  test('applies the yielding detail-dock class when the detail panel is opened on a populated graph', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByTestId('moon-middle')).toHaveClass('moon-middle--action-open');
    });
  });
  ...
  test('renders an overview-style human review strip for populated workflows', async () => {
        ...
        nodes: [
          {
            ...
            agent_tool_plan: { tool_name: 'moon_mutate_field', repeats: 4 },
          },
          {
            node_id: 'node-2',
            kind: 'step',
    ...
    render(<MoonBuildPage workflowId="wf-review" />);
  ...
  test('summarizes branch points without flattening them into the review route', async () => {
    ...
    render(<MoonBuildPage workflowId="wf-branch" />);
  ...
  test('surfaces compile errors inside the compose panel instead of leaving the UI silent', async () => {
    moonBuildPageMocks.materializePlan.mockRejectedValueOnce(new Error('Request timed out after 45s'));
  ...
  test('uses agent CQRS materialize and waits for a multi-step graph before ready handoff', async () => {
    ...
    moonBuildPageMocks.materializePlan.mockResolvedValueOnce({
    ...
    await waitFor(() => {
      expect(moonBuildPageMocks.materializePlan).toHaveBeenCalledWith(
