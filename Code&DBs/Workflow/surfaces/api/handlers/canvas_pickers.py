"""Read-only picker endpoints for Canvas dropdowns.

Backs the source-ref, handoff-target, persistence-target, integration-provider,
and payload-field pickers in Canvas. All endpoints are GET and return compact
lists Canvas renders as ``<datalist>`` suggestions so free-text fields become
...
CANVAS_PICKERS_GET_ROUTES: list[RouteEntry] = [
    (_exact("/api/canvas/pickers/webhook-sources"), _handle_webhook_sources),
    (_exact("/api/canvas/pickers/authorities"), _handle_authorities),
    (_exact("/api/canvas/pickers/integration-providers"), _handle_integration_providers),
    (_exact("/api/canvas/pickers/payload-fields"), _handle_payload_fields),
]


__all__ = ["CANVAS_PICKERS_GET_ROUTES"]
