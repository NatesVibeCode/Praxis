...
def build_execution_setup(
    *,
    ...
    has_workflow = has_multiple_jobs or has_workflow_cues or (
        has_connector_flow and has_build_cues and (has_research or has_review or text_has_any(materialized_prose, "test", "qa"))
    )
    # BUG-3330D2CD + architecture-policy::compile::retrieval-is-the-filter-no-
    # template-fallbacks (2026-04-25). infer_briefing_fields /
    # infer_blocking_inputs / connector_flow_self_scaffolds_inputs emitted a
    ...
    # definition.graph). No silent keyword-triggered substitution.
    ...
    if has_connector_flow and has_workflow and has_build_cues:
        ...
        fanout_workers = 4
    elif has_workflow and has_build_cues and text_has_any(materialized_prose, "workflow", "stage", "staged", "trigger", "route"):
        task_class = "workflow"
        method_key = "staged_execution"
        method_label = "Staged Execution"
        method_summary = (
    ...
    return {
        "setup_version": 1,
        "setup_state": "materialized_preview",
...
# the follow-up commit tracked under roadmap_item.praxis.public.beta.ramp.
# llm.first.infrastructure.wedge.delete.compile.template.fallback.project.
# build.graph.from.definition.graph.
...
def build_build_receipt(
    *,
    ...
    decisions = [
        {"aspect": "method", "choice": _as_text(method.get("key")) or "materialized_setup", "reason": method_reason(execution_setup=execution_setup, materialized_prose=materialized_prose)},
...
def method_reason(*, execution_setup: dict[str, Any], materialized_prose: str) -> str:
    capability_slugs = execution_setup.get("capability_slugs") if isinstance(execution_setup.get("capability_slugs"), list) else []
    method = execution_setup.get("method") if isinstance(execution_setup.get("method"), dict) else {}
    method_key = _as_text(method.get("key"))
    if method_key == "seed_fanout_synthesize":
