...
_DEFAULT_SURFACE_NAME = "materializer"
...
_COMPILE_INDEX_SURFACE_COMPONENTS: tuple[object, ...] = (
    ...
    "materializer.py",
    "definition_materialize_kernel.py",
...
def current_repo_fingerprint(repo_root: str | Path | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        raise _error(
            "materialize_index.repo_fingerprint_unavailable",
...
class MaterializeIndexSnapshot:
    ...
    schema_version: int
    materialize_index_ref: str
    compile_surface_revision: str
    materialize_surface_name: str
    repo_root: str
    ...
    def summary(self) -> dict[str, Any]:
        return {
            ...
            "materialize_index_ref": self.materialize_index_ref,
            "compile_surface_revision": self.compile_surface_revision,
            "materialize_surface_name": self.materialize_surface_name,
...
def refresh_compile_index(
    ...
    *,
    ...
    if conn is None:
        raise _error(
            "materialize_index.authority_missing",
    ...
    surface_name = _require_text(surface_name, field_name="materialize_surface_name")
    ...
    materialize_index_ref = f"materialize_index.{surface_name}.{payload_digest}"
    compile_surface_revision = f"materialize_surface.{surface_name}.{payload_digest}"
    ...
    conn.execute(
        """
        INSERT INTO materialize_index_snapshots (
            materialize_index_ref,
            compile_surface_revision,
            materialize_surface_name,
            schema_version,
            repo_root,
            repo_fingerprint,
            source_fingerprints,
        ...
        ON CONFLICT (materialize_index_ref) DO UPDATE SET
            compile_surface_revision = EXCLUDED.compile_surface_revision,
            materialize_surface_name = EXCLUDED.materialize_surface_name,
            ...
            refresh_count = materialize_index_snapshots.refresh_count + 1,
            updated_at = now()
        """,
        materialize_index_ref,
        compile_surface_revision,
...
def load_compile_index_snapshot(
    ...
    *,
    ...
    if snapshot_ref is not None:
        snapshot_ref = _require_text(snapshot_ref, field_name="materialize_index_ref")
        rows = conn.execute(
            """
            SELECT materialize_index_ref,
                   compile_surface_revision,
                   materialize_surface_name,
                   schema_version,
                   repo_root,
                   ...
                   stale_after_at,
                   refresh_count
              FROM materialize_index_snapshots
             WHERE materialize_index_ref = $1
               AND materialize_surface_name = $2
             LIMIT 1
            """,
            snapshot_ref,
            surface_name,
        ...
        rows = conn.execute(
            ...
            SELECT materialize_index_ref,
                   ...
                   refresh_count
              FROM materialize_index_snapshots
             WHERE materialize_surface_name = $1
    ...
    if not rows:
        raise _error(
            ...
            details={
                ...
                "materialize_surface_name": surface_name,
            },
        )
    ...
    if snapshot_ref is not None and surface_revision is not None:
        if snapshot.compile_surface_revision != surface_revision:
            raise _error(
                "materialize_index.snapshot_surface_mismatch",
                "compile index snapshot revision does not match the pinned revision",
                details={
                    "materialize_index_ref": snapshot.materialize_index_ref,
    ...
    return MaterializeIndexSnapshot(
        ...
        materialize_index_ref=snapshot.materialize_index_ref,
        compile_surface_revision=snapshot.compile_surface_revision,
        materialize_surface_name=snapshot.materialize_surface_name,
        repo_root=snapshot.repo_root,
...
def _snapshot_from_row(row: dict[str, Any]) -> MaterializeIndexSnapshot:
    ...
    materialize_index_ref = _require_text(row.get("materialize_index_ref"), field_name="materialize_index_ref")
    compile_surface_revision = _require_text(
        row.get("compile_surface_revision"),
        field_name="compile_surface_revision",
    )
    materialize_surface_name = _require_text(
        row.get("materialize_surface_name"),
        field_name="materialize_surface_name",
    )
    ...
    try:
        ...
        raise _error(
            ...
            details={"materialize_index_ref": materialize_index_ref},
        ) from exc
    if schema_version != _DEFAULT_SCHEMA_VERSION:
        raise _error(
            "materialize_index.schema_version_unsupported",
