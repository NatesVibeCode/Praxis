...
def materialize_workflow(
    ...
    *,
    ...
    `enable_full_compose` selects which compile pipeline runs:
      ...
      - False: the materialize_prose chain (compile_synthesize → compile_pill_match
    ...
    try:
        ...
        merged_details.setdefault("materialize_preview", preview)
        raise MaterializationError(
            reason_code,
            f"Compile materialization blocked: {reason_code}",
            details=merged_details,
        ) from exc
    mutation["materialize_preview"] = preview
