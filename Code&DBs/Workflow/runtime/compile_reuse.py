...
class CompileReuseError(RuntimeError):
...
def module_surface_manifest(*paths: str | Path) -> dict[str, Any]:
    ...
    if not resolved_paths:
        raise CompileReuseError("module surface revision requires at least one path")
    ...
    for path in resolved_paths:
        try:
            ...
            raise CompileReuseError(f"failed to read module surface {path}: {exc}") from exc
        payload.append(
            {
                "path": str(path),
