...
Sublayer modules:
  materializer_semantic   -- IntentMatcher interaction, embedder resolution, timeout
  materializer_llm        -- LLM calls, response parsing, context building, guards
  materializer_references -- regex-based reference extraction, resolution, job generation
"""
...
import runtime.materializer_components as _compiler_components
import runtime.materializer_output_builders as _compiler_output_builders
...
from runtime.materializer_semantic import (
...
from runtime.materializer_llm import (
...
from runtime.materializer_references import (
...
def materialize_prose(
    prose: str,
    title: str | None = None,
    *,
    enable_llm: bool | None = None,
    materialize_index_ref: str | None = None,
    compile_surface_revision: str | None = None,
    materialize_index_snapshot: MaterializeIndexSnapshot | None = None,
    conn: Any | None = None,
) -> dict[str, Any]:
    ...
    with compile_trace_scope(
        ...
        return _compile_prose_inner(
            ...
            materialize_index_snapshot=materialize_index_snapshot,
            conn=conn,
            _trace=_trace,
        )
...
def _compile_prose_inner(
    *,
    ...
    materialize_index_ref: str | None,
    compile_surface_revision: str | None,
    materialize_index_snapshot: MaterializeIndexSnapshot | None,
    conn: Any | None,
    _trace: Any,
) -> dict[str, Any]:
    clean_prose = prose
    ...
    errors: list[str] = []
    materialize_index = materialize_index_snapshot
    if materialize_index is not None:
        if materialize_index_ref is not None and materialize_index.materialize_index_ref != materialize_index_ref:
            raise RuntimeError(
                "materialize_index.snapshot_mismatch: provided materialize_index_snapshot does not match materialize_index_ref"
            )
        if (
            compile_surface_revision is not None
            and materialize_index.compile_surface_revision != compile_surface_revision
        ):
            raise RuntimeError(
                "materialize_index.snapshot_surface_mismatch: provided materialize_index_snapshot does not match compile_surface_revision"
            )

    try:
        if conn is None:
            conn = _get_connection()
        if materialize_index is None:
            materialize_index = _load_compile_index_snapshot_with_auto_refresh(
                conn,
                materialize_index_ref=materialize_index_ref,
                compile_surface_revision=compile_surface_revision,
            )
        ...
        logger.warning("Failed to load compiler context: %s", exc)
        raise RuntimeError(f"materialize_index.load_failed: {exc}") from exc
    ...
    if conn is not None:
        ...
        try:
            ...
            raise RuntimeError(f"materialize_artifact.reuse_failed: {exc}") from exc
    ...
    materialize_context = materialize_index.to_compile_context()
    catalog = list(materialize_context.get("catalog") or [])
    integrations = list(materialize_context.get("integrations") or [])
    object_types = list(materialize_context.get("object_types") or [])
    capability_catalog = list(materialize_context.get("capabilities") or [])
    route_hints = tuple(tuple(item) for item in materialize_context.get("route_hints") or ())
    ...
    if conn is not None:
        try:
            ...
            raise RuntimeError(f"materialize_artifact.persist_failed: {exc}") from exc
...
def _finalize_compile_result(
    *,
    ...
    conn: Any | None,
    errors: list[str],
    materialize_index: MaterializeIndexSnapshot | None,
    ...
    if used_llm and original_prose:
        ...
        try:
            from runtime.materializer_llm import call_compile_finalize
    ...
    return {
        ...
        "materialize_index": materialize_index.summary() if materialize_index is not None else None,
        "semantic_retrieval": semantic_retrieval,
...
def _definition_compile_provenance(
    *,
    ...
    input_payload = {
        ...
        "title": title or "",
        "llm_requested": llm_requested,
        "materialize_index_ref": materialize_index.materialize_index_ref,
        "compile_surface_revision": materialize_index.compile_surface_revision,
        "repo_fingerprint": materialize_index.repo_fingerprint,
        "source_fingerprints": dict(materialize_index.source_fingerprints),
    }
    return {
        "artifact_kind": "definition",
...
def _fallback_compile_index_snapshot(*, reason: str) -> MaterializeIndexSnapshot:
    ...
    return MaterializeIndexSnapshot(
        schema_version=1,
        materialize_index_ref="materialize_index:fallback",
    ...
    *,
    ...
    try:
        return load_compile_index_snapshot(
            conn,
            snapshot_ref=materialize_index_ref,
        ...
        if materialize_index_ref is not None or compile_surface_revision is not None:
            raise
...
def _compiler_surface_revision() -> str:
    ...
    if _COMPILER_SURFACE_REVISION_CACHE is None:
        _COMPILER_SURFACE_REVISION_CACHE = module_surface_revision(
            __file__,
            Path(__file__).with_name("definition_materialize_kernel.py"),
        )
