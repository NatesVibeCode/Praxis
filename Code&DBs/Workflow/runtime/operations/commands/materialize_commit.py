...
`authority_operation_receipts` row + emits `materialize.committed` to
...
class MaterializeCommand(BaseModel):
    ...
    enable_full_compose: bool | None = Field(
        ...
        description=(
            ...
            "through materialize_prose (compile_synthesize → compile_pill_match → "
...
def handle_materialize_commit(
    command: MaterializeCommand, subsystems: Any
) -> dict[str, Any]:
    ...
    `materialize.committed` to `authority_events` (event_required=TRUE on the
