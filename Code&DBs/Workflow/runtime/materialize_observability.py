...
distinct ways:
  ...
  1. LLM was requested but routing missed (no llm_task primary for the task
     type). materialize_prose falls through to the deterministic path.
  ...
  5. materialize_prose raised before persisting — caller sees the exception but
     no durable record of what was attempted.

This module records each materialize_prose invocation as one ``materialize_runs``
...
read recent rows; use ``materialize_health`` view for aggregate degradation
signals.
"""
...
class MaterializeRunTrace:
    """Mutable accumulator for one materialize_prose invocation's provenance.
    ...
    durable row to ``materialize_runs``.
    """

    materialize_run_id: str = field(default_factory=lambda: str(uuid4()))
...
def begin_compile_trace(
    *,
    ...
    """Open a new trace at the start of materialize_prose.
...
def record_compile_trace(conn: Any, trace: MaterializeRunTrace) -> None:
    """Persist one materialize_runs row.
    ...
    try:
        ...
        logger.warning("materialize_runs trace record failed: %s", exc)
