...
def build_definition(
    *,
    ...
    narrative_blocks = build_narrative_blocks(materialized_prose, references, capabilities)
    trigger_intent = build_trigger_intent(materialized_prose, references, narrative_blocks)
    ...
    definition = {
        ...
        "source_prose": materialized_source_prose(definition_graph, fallback=source_prose),
        "materialized_prose": materialized_prose_projection(definition_graph, fallback=materialized_prose),
...
def materialize_definition(definition: dict[str, Any]) -> dict[str, Any]:
    ...
    materialized_prose = as_text(materialized.get("materialized_prose"))
    ...
    if not isinstance(definition_graph, dict):
        effective_narrative_blocks = narrative_blocks or build_narrative_blocks(materialized_prose, references, capabilities)
    ...
    materialized["source_prose"] = materialized_source_prose(definition_graph, fallback=source_prose)
    materialized["materialized_prose"] = materialized_prose_projection(definition_graph, fallback=materialized_prose)
...
def materialized_source_prose(definition_graph: dict[str, Any], *, fallback: str = "") -> str:
...
def materialized_prose_projection(definition_graph: dict[str, Any], *, fallback: str = "") -> str:
    metadata = definition_graph.get("metadata") if isinstance(definition_graph, dict) else {}
    if isinstance(metadata, dict):
        value = as_text(metadata.get("materialized_prose"))
        if value:
            return value
...
def detect_triggers(materialized_prose: str, references: list[dict[str, Any]]) -> list[dict[str, Any]]:
    triggers: list[dict[str, Any]] = []
    seen: set[tuple[str, str, str]] = set()
    sentences = split_sentences(materialized_prose)
...
def build_narrative_blocks(
    ...
    sentences = split_sentences(materialized_prose)
    if not sentences and materialized_prose.strip():
        stripped = materialized_prose.strip()
        start = materialized_prose.find(stripped)
...
def build_trigger_intent(
    ...
    trigger_intent: list[dict[str, Any]] = []
    for index, trigger in enumerate(detect_triggers(materialized_prose, references), start=1):
