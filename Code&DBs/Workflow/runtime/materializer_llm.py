...
from runtime.definition_materialize_kernel import split_sentences as _kernel_split_sentences

logger = logging.getLogger(__name__)

_APP_COMPILE_TASK_ROUTE = "auto/materialize"
...
def derive_title(prose: str, materialized_prose: str) -> str:
    source = materialized_prose or prose
    if not source:
...
def call_llm_compile(prose: str, context: str, *, conn: Any = None, hydrate_env: Any = None, get_connection: Any = None) -> dict[str, Any]:
    ...
    pill_match_raw = _call_compile_sub_task(
        task_type="materialize_pill_match",
...
def call_compile_finalize(
    *,
    ...
    `task_type='materialize_finalize'` so the picker resolves voters from data,
...
def build_refinement_summary(
    *,
    ...
    materialized_prose = _as_text(compiled.get("prose")) or source_prose
    authority = _as_text(compiled.get("authority"))
    sla = compiled.get("sla") if isinstance(compiled.get("sla"), dict) else {}
    materialized_capabilities = _as_string_list(compiled.get("capabilities"))
    materially_changed = (
        materialized_prose.strip() != source_prose.strip()
        or bool(authority)
        or bool(sla)
        or bool(materialized_capabilities)
    )
