...
def materialize_spec(
    ...
    *,
    ...
    if conn is None:
        raise CapabilityCatalogError("materialize_spec requires Postgres authority for capability selection")
...
def materialize_intent_from_file(file_path: str) -> tuple[Intent | None, list[str]]:
...
def materialize_prompt_launch_spec(
    *,
    prompt: str,
    provider_slug: str | None = None,
    model_slug: str | None = None,
    ...
    """Materialize a prompt launch into the inline workflow.submit shape.
...
compile_prompt_launch_spec = materialize_prompt_launch_spec
...
class PlanPacket:
    ...
    at ``materialize_plan`` time per fail-closed-at-compile-no-silent-defaults.
...
class LaunchReceipt:
    ...
    warnings: list[str]
    paid_model_access: list[dict[str, Any]] = field(default_factory=list)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "paid_model_access": list(self.paid_model_access),
        }
...
def _bind_packet_data_pills(
    ...
    *,
    ...
    Shared entry point for ``materialize_plan`` (attaches pills to the job dict
...
def _build_packet_map_entry(
    *,
    ...
    - ``alternatives_considered`` — list of MaterializeDecision dicts captured
      during ``materialize_plan``'s per-packet loop (Foundation slice for the
      materialize + Review front door). Sourced from ``job['materialize_decisions']``
      via the same job-dict mirror pattern as ``data_pills``. Empty list
      means no judgment call was made for this packet.
    ...
    return {
        ...
        # Materialize-time judgment calls and the runner-ups the resolver
        # had on hand (Foundation slice for the materialize + Review front
        # door — see runtime.materialize.review_payload). Empty list means
        # no judgment call was made for this packet, NOT data missing.
        "alternatives_considered": list(job.get("materialize_decisions") or []),
    }
...
class UnresolvedSourceRefError(ValueError):
    ...
    Foundation slice for compile + Review front door also populates
    ``unresolved_entries`` with the runner-up shape
    ``[{ref, available_options: [{ref, reason_not_picked}, ...]}]`` —
    the chat-model Review handler reads this seam to explain *what
    prefixes the compiler had on hand* when the supplied ref didn't
    match. ``unresolved_refs`` stays as the bare-string list for legacy
    consumers (``typed_gap_events``, existing tests).
    """
    ...
    def __init__(self, unresolved_refs: list[str]) -> None:
        ...
        self.unresolved_entries: list[dict[str, Any]] = [
            {
                "ref": ref,
                "available_options": [
                    {
                        "ref": prefix,
                        "reason_not_picked": "prefix mismatched supplied ref",
                    }
...
def _plan_packets_from_source_refs(
    ...
    *,
    ...
    failure shape of :class:`MaterializePlanError`. Per-kind resolvers that
    ...
    if has_source_refs:
        if conn is None:
            raise ValueError(
                ...
                "materialize_plan / propose_plan / launch_plan"
            )
...
def _enrich_prompt_with_context(
    ...
    *,
    ...
    The stage-template prompt that ``materialize_spec`` emits is intentionally
...
class UnresolvedWriteScopeError(ValueError):
    ...
    def __init__(self, unresolved_writes: list[dict[str, Any]]) -> None:
        # Foundation slice for compile + Review front door: each entry gains
        # an ``available_options`` field — empty list at Foundation because
        # write-scope has no resolver offering runners-up today. Field exists
        # so the future scope_resolver lane can populate it without a shape
        # change at the consumer seam.
        self.unresolved_writes = [
            {**dict(entry), "available_options": list(entry.get("available_options") or [])}
        ...
        for entry in self.unresolved_writes:
            lines.append(
                f"  - packet[{entry['index']}] label={entry['label']!r} "
...
class UnresolvedStageError(ValueError):
    ...
    def __init__(self, unresolved_stages: list[dict[str, Any]]) -> None:
        ...
        # Foundation slice for compile + Review front door: lift the
        # already-computed admitted-stage list into each entry's
        # ``available_options``. The error already knew the admitted set;
        # now it ships it on the structured payload so the chat-model
        # Review handler can explain "you asked for stage X; here is what
        # was admitted" without re-deriving the set.
        stage_options = [
            {
                "ref": stage_name,
                "reason_not_picked": "admitted in _STAGE_TEMPLATES but not chosen",
            }
            for stage_name in known
        ]
        self.unresolved_stages = [
            {**dict(entry), "available_options": list(stage_options)}
        ...
        for entry in self.unresolved_stages:
            lines.append(
                f"  - packet[{entry['index']}] label={entry['label']!r} "
    ...
    return {
        ...
        **manifest_payload,
        "hardening_report": {
            ...
            "source": "runtime.spec_materializer.materialize_plan",
            "job_count": len(jobs),
...
def materialize_plan(
    ...
    *,
    ...
    before raising :class:`MaterializePlanError`. No partial spec_dict returned.
    ...
    workflow_id out, so repeated materialize_plan calls produce the same spec
    ...
    for index, packet in enumerate(plan_obj.packets):
        ...
        # Foundation slice for the compile + Review front door — accumulate
        # the judgment calls the compiler makes for this packet so
        # ``_build_packet_map_entry`` surfaces them as
        # ``alternatives_considered``. Empty list means no judgment call
        # was made; the chat-model Review handler treats that as silence,
        # not as missing data. Mirrors the ``data_pills`` job-dict pattern
        # below — additive, no signature changes.
        ...
        if packet.stage not in _STAGE_TEMPLATES:
            ...
            continue
        # Stage resolved — record the decision. Foundation stub: alternatives
        # stay empty here on the success path (the runner-up shape is what
        # ``UnresolvedStageError.unresolved_stages[i]['available_options']``
        # ships on the failure path). A later lane can populate alternatives
        # with structurally-admissible-but-not-chosen stages once the stage
        # picker exposes its candidate set.
        ...
        try:
            compiled, warnings = materialize_spec(intent_dict, conn=conn)
        except Exception as exc:
            failures.append(
                {
                    "index": index,
        ...
        # Agent resolved — record the decision. Foundation stub: alternatives
        # stay empty here; the agent picker exposes its candidate set in a
        # later lane (today the agent string is set deterministically by
        # ``_packet_to_job`` from the stage template, so there is no real
        # judgment call on the success path).
        packet_decisions.append(
            {
        ...
        # Lift any ambiguous data-pill candidates into a structured
        # ``data_pill_binding`` decision so the Review handler can explain
        # "this prose had multiple object.field matches; here are the
        # candidates we saw." When ``bound`` is empty for an ambiguous
        # span, ``chosen`` stays None — legal under
        # ``CompileDecision.chosen: str | None``.
        for ambiguous_entry in job["data_pills"].get("ambiguous") or ():
            matched_span = ambiguous_entry.get("matched_span") or ""
            candidate_refs = [
                f"{cand.get('object_kind','')}.{cand.get('field_path','')}"
                for cand in (ambiguous_entry.get("candidates") or [])
            ]
            packet_decisions.append(
                {
                    ...
                    "alternatives": [
                        {
                            "ref": ref,
                            "reason_not_picked": "ambiguous match against intent prose",
                        }
                        for ref in candidate_refs
                    ],
                    "confidence": None,
                    "notes": f"matched_span={matched_span!r}",
                }
            )
        # Attach the accumulated decisions to the job dict, mirroring the
        # ``data_pills`` pattern above. ``_build_packet_map_entry`` reads
        # this seam at line 1140-ish to surface ``alternatives_considered``
        # on each packet_map entry.
        job["materialize_decisions"] = packet_decisions
    ...
    if unresolved_writes:
        ...
        if conn is not None:
            try:
                ...
                emit_typed_gaps_for_compile_errors(
                    conn, err_write, source_ref=f"materialize_plan:{plan_obj.name}"
                )
            except Exception:
    ...
    if unresolved_stages:
        ...
        if conn is not None:
            try:
                ...
                emit_typed_gaps_for_compile_errors(
                    conn, err_stage, source_ref=f"materialize_plan:{plan_obj.name}"
                )
            except Exception:
                pass
        raise err_stage
    if failures:
        raise MaterializePlanError(failures)
    if ref_conflicts:
        raise ValueError(
            "materialize_plan resolved conflicting execution authority refs: "
            + "; ".join(ref_conflicts)
        )
...
class ProposedPlan:
    ...
    paid_model_requirements: list[dict[str, Any]] = field(default_factory=list)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "paid_model_requirements": list(self.paid_model_requirements),
        }
...
class PaidModelApprovalError(ValueError):
    """Raised when paid model requirements are not explicitly approved."""
...
def _paid_selector_key(selector: Mapping[str, Any]) -> tuple[str, str, str, str, str, str]:
    from runtime.paid_model_access import require_exact_paid_selector
...
def _paid_selector_from_requirement(requirement: Mapping[str, Any]) -> dict[str, Any]:
    return {
        "runtime_profile_ref": requirement.get("runtime_profile_ref"),
        "job_type": requirement.get("job_type"),
        "transport_type": requirement.get("transport_type"),
        "adapter_type": requirement.get("adapter_type"),
        "provider_slug": requirement.get("provider_slug"),
        "model_slug": requirement.get("model_slug"),
    }


def _dedupe_paid_model_requirements(
    requirements: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    deduped: dict[tuple[str, str, str, str, str, str], dict[str, Any]] = {}
    for requirement in requirements:
        key = _paid_selector_key(_paid_selector_from_requirement(requirement))
        existing = deduped.get(key)
        if existing is None:
            labels = []
            label = str(requirement.get("label") or requirement.get("job_label") or "").strip()
            if label:
                labels.append(label)
            deduped[key] = {**dict(requirement), "labels": labels}
            continue
        label = str(requirement.get("label") or requirement.get("job_label") or "").strip()
...
def _paid_approval_acknowledged(approval: Mapping[str, Any]) -> bool:
    raw = approval.get("acknowledged")
    if raw is None:
        raw = approval.get("paid_model_acknowledged")
    if raw is None:
        raw = approval.get("approved")
    if isinstance(raw, bool):
        return raw
    return str(raw or "").strip().lower() in {"1", "true", "yes", "y", "ack", "acknowledged", "approved"}


def _paid_model_approval_index(
    approvals: list[Mapping[str, Any]],
) -> dict[tuple[str, str, str, str, str, str], Mapping[str, Any]]:
    indexed: dict[tuple[str, str, str, str, str, str], Mapping[str, Any]] = {}
    for approval in approvals:
        key = _paid_selector_key(approval)
        if key in indexed:
            raise PaidModelApprovalError(
                "duplicate paid_model_approvals entry for exact selector "
                f"{'/'.join(key)}"
            )
...
def _paid_model_requirements_for_proposal(
    conn: Any,
    *,
    ...
    runtime_profile_ref = str(
        spec_dict.get("runtime_profile_ref")
        or ((preview.get("workspace") or {}) if isinstance(preview.get("workspace"), dict) else {}).get("runtime_profile_ref")
        or ""
    ).strip()
    ...
    preview_by_label = {
        str(job.get("label") or "").strip(): dict(job)
        for job in (preview.get("jobs") or [])
    ...
    for job in spec_dict.get("jobs") or []:
        ...
        preview_job = preview_by_label.get(label, {})
        agent_slug = str(
            preview_job.get("resolved_agent")
            or preview_job.get("requested_agent")
            or job.get("agent")
            or ""
        ).strip()
        if not agent_slug or agent_slug.startswith("auto/") or "/" not in agent_slug:
            continue
        ...
        if not job_type:
            continue
        adapter_type = str(preview_job.get("adapter_type") or job.get("adapter_type") or "").strip()
        if not adapter_type:
            try:
                adapter_type = resolve_default_adapter_type(provider_slug)
            except Exception:
        ...
        transport_type = transport_type_for_adapter(adapter_type)
    ...
    return _dedupe_paid_model_requirements(requirements)


def _grant_paid_model_leases_for_approval(
    conn: Any,
    *,
    requirements: list[dict[str, Any]],
    paid_model_approvals: list[Mapping[str, Any]],
    approved_by: str,
    approved_at: datetime,
    proposal_hash: str,
    approval_note: str | None,
    provider_freshness: Mapping[str, Any] | None,
) -> list[dict[str, Any]]:
    deduped = _dedupe_paid_model_requirements(requirements)
    if not deduped:
        if paid_model_approvals:
            raise PaidModelApprovalError(
                "paid_model_approvals were supplied, but the proposal has no paid model requirements"
            )
        return []
    if conn is None:
        raise PaidModelApprovalError(
            "paid model approval requires Postgres authority so exact one-run leases can be created"
        )
    approval_index = _paid_model_approval_index(paid_model_approvals)
    requirement_keys = {
        _paid_selector_key(_paid_selector_from_requirement(requirement))
        for requirement in deduped
    }
    missing = []
    for requirement in deduped:
        selector = _paid_selector_from_requirement(requirement)
        key = _paid_selector_key(selector)
        approval = approval_index.get(key)
        if approval is None or not _paid_approval_acknowledged(approval):
            missing.append(selector)
    extras = [
        {
            "runtime_profile_ref": key[0],
            "job_type": key[1],
            "transport_type": key[2],
            "adapter_type": key[3],
            "provider_slug": key[4],
            "model_slug": key[5],
        }
        for key in approval_index
        if key not in requirement_keys
    ]
    if missing or extras:
        raise PaidModelApprovalError(
            "paid model approval must acknowledge every exact paid route and no extras",
        )

    route_truth_ref = str((provider_freshness or {}).get("route_truth_ref") or "").strip() or None
    expires_at = (approved_at + timedelta(minutes=30)).isoformat()

    def _write(write_conn: Any) -> list[dict[str, Any]]:
        from runtime.paid_model_access import (
        ...
        for index, requirement in enumerate(deduped, start=1):
            selector = _paid_selector_from_requirement(requirement)
            approval = approval_index[_paid_selector_key(selector)]
            approval_ref = str(
                approval.get("approval_ref")
                or f"paid-model-approval.{proposal_hash[:16]}.{index}"
            ).strip()
            leases.append(
                grant_paid_model_lease(
                    write_conn,
                    ...
                    cost_posture=dict(requirement.get("cost_posture") or approval.get("cost_posture") or {}),
                    route_truth_ref=route_truth_ref,
                    decision_ref=str(requirement.get("decision_ref") or PAID_MODEL_POLICY_DECISION_REF),
                )
            )
        return leases
...
def _bind_paid_model_leases_before_submit(
    conn: Any,
    *,
    lease_ids: list[str],
    run_id: str,
    ...
    if not lease_ids:
        return []
    from runtime.paid_model_access import bind_paid_model_leases_to_run

    return bind_paid_model_leases_to_run(conn, lease_ids=lease_ids, run_id=run_id)


def _paid_model_receipt_rows(
    *,
    requirements: list[dict[str, Any]],
    leases: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    requirements_by_key = {
        _paid_selector_key(_paid_selector_from_requirement(requirement)): requirement
        for requirement in requirements
    }
    rows: list[dict[str, Any]] = []
    for lease in leases:
        key = _paid_selector_key(lease)
        requirement = requirements_by_key.get(key, {})
        rows.append(
            {
                "provider_slug": lease.get("provider_slug"),
                "model_slug": lease.get("model_slug"),
                "transport_type": lease.get("transport_type"),
                "adapter_type": lease.get("adapter_type"),
                "job_type": lease.get("job_type"),
                "runtime_profile_ref": lease.get("runtime_profile_ref"),
                "cost_posture": lease.get("cost_posture") or requirement.get("cost_posture"),
                "lease_scope": "one_workflow_run",
                "lease_id": lease.get("lease_id"),
                "lease_state": lease.get("status"),
                "lease_expires_at": lease.get("expires_at"),
                "approval_ref": lease.get("approval_ref"),
                "proposal_hash": lease.get("proposal_hash"),
                "bound_run_id": lease.get("bound_run_id"),
...
def propose_plan(
    ...
    *,
    ...
    """
    spec_dict, warnings_all = materialize_plan(plan, conn=conn, workdir=workdir)
    plan_obj = _coerce_plan(plan, conn=conn)
    ...
    for packet, job in zip(plan_obj.packets, spec_dict["jobs"]):
        # materialize_plan already bound data pills via _bind_packet_data_pills
    ...
    paid_model_requirements = _paid_model_requirements_for_proposal(
        conn,
        spec_dict=spec_dict,
        preview=preview,
    )
    if paid_model_requirements:
        paid_labels = sorted(
            {
                str(requirement.get("label") or "")
                for requirement in paid_model_requirements
        ...
        warnings_all = [
            *warnings_all,
            (
                f"{len(paid_model_requirements)} paid model route(s) require "
                f"explicit paid_model_approvals before launch: {paid_labels}"
            ),
        ]
    ...
    return ProposedPlan(
        ...
        paid_model_requirements=paid_model_requirements,
    )
...
class ApprovedPlan:
    ...
    paid_model_leases: list[dict[str, Any]] = field(default_factory=list)
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "paid_model_leases": list(self.paid_model_leases),
        }
...
def approve_proposed_plan(
    ...
    *,
    approved_by: str,
    approval_note: str | None = None,
    paid_model_approvals: list[Mapping[str, Any]] | None = None,
    conn: Any | None = None,
) -> ApprovedPlan:
    ...
    paid_model_leases = _grant_paid_model_leases_for_approval(
        conn,
        requirements=list(proposed.paid_model_requirements or []),
        paid_model_approvals=list(paid_model_approvals or []),
        approved_by=approver,
        approved_at=approved_at_dt,
        proposal_hash=proposal_hash,
        approval_note=str(approval_note).strip() if approval_note else None,
        provider_freshness=proposed.provider_freshness,
    )
    ...
    return ApprovedPlan(
        ...
        proposal_hash=proposal_hash,
        approval_note=str(approval_note).strip() if approval_note else None,
        paid_model_leases=paid_model_leases,
    )
...
def launch_approved(
    ...
    *,
    ...
    receipt = launch_proposed(
        ...
        paid_model_leases=approved.paid_model_leases,
    )
    return receipt
...
def launch_proposed(
    ...
    *,
    ...
    requested_by_kind: str = "workflow",
    requested_by_ref: str | None = None,
    paid_model_leases: list[dict[str, Any]] | None = None,
) -> LaunchReceipt:
    ...
    lease_ids = [
        ...
        for lease in (paid_model_leases or [])
        if str(lease.get("lease_id") or "").strip()
    ]
    if proposed.paid_model_requirements and not lease_ids:
        raise PaidModelApprovalError(
            "paid model launch requires one-run leases created by approve_proposed_plan"
        )
    submit_result: dict[str, Any]
    bound_paid_model_leases: list[dict[str, Any]] = []
    forced_run_id = f"workflow_{uuid.uuid4().hex[:12]}" if lease_ids else None

    def _submit(submit_conn: Any) -> dict[str, Any]:
        nonlocal bound_paid_model_leases
        if forced_run_id is not None:
            bound_paid_model_leases = _bind_paid_model_leases_before_submit(
                submit_conn,
                lease_ids=lease_ids,
                run_id=forced_run_id,
            )
        return submit_workflow_command(
            submit_conn,
            requested_by_kind=requested_by_kind,
            ...
            run_id=forced_run_id,
            force_fresh_run=bool(forced_run_id),
            dispatch_reason=f"launch_proposed:{proposed.spec_name}",
        )

    transaction = getattr(conn, "transaction", None)
    if forced_run_id is not None and callable(transaction):
        with transaction() as tx:
            submit_result = _submit(tx)
            run_id = _ensure_run_id_or_raise(submit_result, spec_name=proposed.spec_name)
    else:
        submit_result = _submit(conn)
        run_id = _ensure_run_id_or_raise(submit_result, spec_name=proposed.spec_name)
    ...
    return LaunchReceipt(
        ...
        warnings=warnings,
        paid_model_access=_paid_model_receipt_rows(
            requirements=list(proposed.paid_model_requirements or []),
            leases=bound_paid_model_leases or list(paid_model_leases or []),
        ),
    )
