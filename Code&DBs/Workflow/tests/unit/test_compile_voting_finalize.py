"""Tests for the voting helper + compile_finalize stage in compiler_llm.
...
def test_resolve_top_k_voters_picks_best_affinity_match(monkeypatch) -> None:
    ...
    import runtime.compiler_llm as compiler_llm
...
def _install_voter_pool(monkeypatch, *, voter_models: list[str]):
    ...
    monkeypatch.setattr(
        compiler_llm, "resolve_top_k_voters", lambda *a, **kw: pool[: kw.get("k", len(pool))]
    )
...
def test_finalize_skips_already_accepted_bindings(monkeypatch) -> None:
    ...
    monkeypatch.setattr(compiler_llm, "_call_voting_sub_task", _explode)

    result = compiler_llm.call_compile_finalize(
...
def test_finalize_resolves_blocking_binding_to_chosen_candidate(monkeypatch) -> None:
    ...
    monkeypatch.setattr(compiler_llm, "_call_voting_sub_task", _fake_voting)
...
def test_finalize_parser_handles_null_resolved_target(monkeypatch) -> None:
    ...
    assert compiler_llm._parse_finalize_response(raw_null) is None
    assert compiler_llm._parse_finalize_response(raw_explicit_null) is None
    assert compiler_llm._parse_finalize_response(raw_empty) is None
    assert compiler_llm._parse_finalize_response(raw_real) == "@notifications/send"
    assert compiler_llm._parse_finalize_response("not-json") is None
