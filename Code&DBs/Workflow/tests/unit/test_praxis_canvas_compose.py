...
from surfaces.mcp.tools import canvas


def test_praxis_canvas_compose_dispatches_compile_materialize_then_reads_build(monkeypatch) -> None:
    ...
    monkeypatch.setattr(canvas, "execute_operation_from_subsystems", _dispatch)

    result = canvas.tool_praxis_canvas(
        {
            "action": "compose",
            "intent": "Search issues, draft summary, notify Slack",
            "plan_name": "Canvas delivery",
            "enable_llm": True,
    ...
    assert calls[1][1] == {"workflow_id": "wf_canvas"}
    assert result["ok"] is True
    assert result["workflow_id"] == "wf_canvas"
