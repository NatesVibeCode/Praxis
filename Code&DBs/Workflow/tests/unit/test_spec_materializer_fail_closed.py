...
matching the MaterializePlanError shape.
"""
...
class _StubConn:
    """Minimal conn stub — spec_materializer.materialize_plan doesn't touch the DB
...
def test_unresolved_stage_takes_precedence_over_other_compile_failures():
    ...
    # Stage is mystery (unresolved); if materialize_spec were also called, it would
...
def test_known_stages_compile_successfully():
    ...
    for stage in sorted(_STAGE_TEMPLATES.keys()):
        ...
        # MaterializePlanError for some OTHER reason (DB-less stub), but not for stage.
        try:
            materialize_plan(plan, conn=_StubConn())
        except UnresolvedStageError:
            pytest.fail(f"stage={stage!r} should be admitted, raised UnresolvedStageError")
        except Exception:
            # Other failures (e.g., MaterializePlanError from missing authority) are OK —
...
def test_non_empty_write_passes_pre_validation():
    ...
    # May raise for OTHER reasons (e.g., MaterializePlanError via stub conn),
...
# ---------------------------------------------------------------------------
# materialize_plan emits typed_gap.created events at error raise (Phase 1.6 wiring)
# ---------------------------------------------------------------------------


class _EventRecordingConn:
    """Stub conn that captures every execute() call — used to verify
    materialize_plan emits system_events before raising."""
...
def test_compile_plan_emits_typed_gap_events_for_write_scope_errors():
    ...
    with pytest.raises(UnresolvedWriteScopeError):
        materialize_plan(plan, conn=conn)
...
def test_compile_plan_emission_is_best_effort_even_if_events_fail(monkeypatch):
    """If the emit helper raises (e.g., degraded system_events), materialize_plan
    ...
    with pytest.raises(UnresolvedWriteScopeError):
        materialize_plan(plan, conn=_EventRecordingConn())
