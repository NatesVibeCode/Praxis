...
def _compile_index_snapshot() -> materialize_index.MaterializeIndexSnapshot:
    repo_info = {
    ...
    return materialize_index.MaterializeIndexSnapshot(
        schema_version=1,
        materialize_index_ref="materialize_index.materializer.test",
        compile_surface_revision="materialize_surface.materializer.test",
...
def test_compile_prose_reuses_definition_artifact_on_exact_input_match(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    first = compiler.materialize_prose(
    ...
    second = compiler.materialize_prose(
        "Review the workflow output carefully.",
        conn=conn,
        materialize_index_snapshot=snapshot,
    )
