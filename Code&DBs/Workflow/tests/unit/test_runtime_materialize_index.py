...
class _CompileIndexConn:
    ...
    def execute(self, query: str, *params: Any) -> list[dict[str, Any]]:
        ...
        if "INSERT INTO materialize_index_snapshots" in query:
            self.inserted_row = {
                "materialize_index_ref": params[0],
                "compile_surface_revision": params[1],
                "materialize_surface_name": params[2],
            ...
            return []
        if "FROM materialize_index_snapshots" in query:
...
def _snapshot_row(
    *,
    ...
    materialize_surface_name: str = "materializer",
    ...
    payload = {
        ...
        "surface_manifest": {
            "repo_root": repo_root,
            "surface_name": "materializer",
            "surface_revision": "surface_materializer_manifest_test",
            "tracked_files": ["Code&DBs/Workflow/runtime/materialize_index.py"],
            "file_fingerprints": {
                "Code&DBs/Workflow/runtime/materialize_index.py": "surface-file-fingerprint"
            },
        },
        "source_fingerprints": {
    ...
    return {
        ...
        "materialize_surface_name": materialize_surface_name,
        "schema_version": 1,
        "repo_root": repo_root,
        "repo_fingerprint": repo_fingerprint,
...
def test_current_compile_surface_manifest_tracks_live_authority_modules() -> None:
    manifest = materialize_index.current_compile_surface_manifest(Path(_REPO_ROOT))
    tracked_files = set(manifest["tracked_files"])

    assert "Code&DBs/Workflow/runtime/materialize_index.py" in tracked_files
    assert "Code&DBs/Workflow/runtime/materialize_reuse.py" in tracked_files
    assert "Code&DBs/Workflow/runtime/materializer.py" in tracked_files
    assert "Code&DBs/Workflow/runtime/definition_materialize_kernel.py" in tracked_files
...
def test_refresh_compile_index_materializes_durable_snapshot(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(materialize_index, "sync_integration_registry", lambda _conn: 1)
    monkeypatch.setattr(
        materialize_index,
        "sync_reference_catalog",
        lambda _conn, **kwargs: 1,
    )
    monkeypatch.setattr(materialize_index, "sync_capability_catalog", lambda _conn, **kwargs: 1)
    monkeypatch.setattr(materialize_index, "_load_integrations", lambda _conn: source_rows["integrations"])
    monkeypatch.setattr(materialize_index, "_load_object_types", lambda _conn: source_rows["object_types"])
    monkeypatch.setattr(materialize_index, "_load_reference_catalog", lambda _conn: source_rows["catalog"])
    monkeypatch.setattr(materialize_index, "_load_compiler_route_hints", lambda _conn: source_rows["route_hints"])
    monkeypatch.setattr(materialize_index, "load_capability_catalog", lambda _conn: source_rows["capabilities"])
    monkeypatch.setattr(
        materialize_index,
    ...
    snapshot = materialize_index.refresh_compile_index(
        conn,
    ...
    assert snapshot.materialize_index_ref.startswith("materialize_index.materializer.")
    assert snapshot.compile_surface_revision.startswith("materialize_surface.materializer.")
    assert snapshot.materialize_surface_name == "materializer"
    assert snapshot.repo_fingerprint == "repo-fingerprint"
    assert snapshot.surface_manifest["surface_revision"] == "surface_materializer_manifest_test"
    ...
    assert any(
        query.strip().startswith("SELECT materialize_index_ref")
        for query, _ in conn.executed
    )
...
def test_load_compile_index_snapshot_returns_latest_fresh_snapshot() -> None:
    ...
    snapshot = materialize_index.load_compile_index_snapshot(
        conn,
        surface_name="materializer",
        require_fresh=True,
        repo_root=None,
    ...
    assert snapshot.materialize_index_ref == "materialize_index.materializer.test"
    assert snapshot.compile_surface_revision == "materialize_surface.materializer.test"
    ...
    assert conn.executed[0][0].strip().startswith("SELECT materialize_index_ref")


def test_load_compile_index_snapshot_missing_fails_closed() -> None:
    conn = _CompileIndexConn(rows=[])

    with pytest.raises(materialize_index.MaterializeIndexAuthorityError) as exc_info:
        materialize_index.load_compile_index_snapshot(
            conn,
            surface_name="materializer",
            require_fresh=True,
            repo_root=None,
    ...
    assert exc_info.value.reason_code == "materialize_index.snapshot_missing"
...
def test_load_compile_index_snapshot_stale_fails_closed() -> None:
    ...
    assert exc_info.value.reason_code == "materialize_index.snapshot_stale"
...
def test_load_compile_index_snapshot_stales_when_surface_manifest_changes(
    ...
    monkeypatch.setattr(
        ...
        lambda repo_root=None: {
            ...
            "tracked_files": ["Code&DBs/Workflow/runtime/materialize_index.py"],
            "file_fingerprints": {
                "Code&DBs/Workflow/runtime/materialize_index.py": "surface-file-fingerprint-changed"
            },
        },
    )
