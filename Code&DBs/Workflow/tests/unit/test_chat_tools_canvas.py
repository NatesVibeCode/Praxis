"""Unit tests for the Canvas graph authoring chat tools.

The five ``canvas_*`` chat tools are thin wrappers over registered CQRS
...
def test_canvas_get_build_dispatches_workflow_build_get_and_returns_compact_summary(monkeypatch):
    ...
    result = chat_tools.execute_tool("canvas_get_build", {"workflow_id": "wf_abc"}, pg_conn=object(), repo_root="/tmp")
...
def test_canvas_get_build_requires_workflow_id():
    result = chat_tools.execute_tool("canvas_get_build", {}, pg_conn=object(), repo_root="/tmp")
    assert result["type"] == "error"
    assert "workflow_id is required" in result["summary"]


def test_canvas_get_build_surfaces_dispatch_errors(monkeypatch):
    def _explode(*args, **kwargs):
        raise RuntimeError("postgres unreachable")

    monkeypatch.setattr(chat_tools, "_dispatch_op", _explode)
    result = chat_tools.execute_tool("canvas_get_build", {"workflow_id": "wf_x"}, pg_conn=object(), repo_root="/tmp")
    assert result["type"] == "error"
...
def test_canvas_compose_materializes_and_reads_build_when_no_workflow_id(monkeypatch):
    """Path A: no active Canvas workflow -> compile_materialize creates one."""
    recorder = _patch_dispatch(monkeypatch, {
        "compile_materialize": {
            "ok": True,
...
def test_canvas_compose_materializes_existing_workflow_from_context(monkeypatch):
    """Path B: active Canvas workflow in context -> compile_materialize targets it."""
    recorder = _patch_dispatch(monkeypatch, {
        "compile_materialize": {
            "ok": True,
        ...
        "workflow_build_get": {
            "workflow": {"id": "wf_active", "name": "canvas-demo"},
    ...
    assert payload["targeted_via"] == "canvas_context"


def test_canvas_compose_requires_intent():
    result = chat_tools.execute_tool("canvas_compose", {}, pg_conn=object(), repo_root="/tmp")
    assert result["type"] == "error"


def test_canvas_compose_surfaces_materialize_blocker(monkeypatch):
    recorder = _patch_dispatch(monkeypatch, {
        "compile_materialize": {
            "ok": False,
            "error": "empty graph",
...
def test_canvas_mutate_field_dispatches_workflow_build_mutate_with_subpath(monkeypatch):
    recorder = _patch_dispatch(monkeypatch, {
...
def test_canvas_mutate_field_validates_inputs():
    result = chat_tools.execute_tool(
        "canvas_mutate_field",
...
def test_canvas_suggest_next_calls_get_then_suggest(monkeypatch):
    recorder = _patch_dispatch(monkeypatch, {
        "workflow_build_get": {
            "workflow_id": "wf_abc",
            "build_graph": {"nodes": [{"node_id": "n1"}], "edges": []},
...
def test_canvas_launch_dispatches_launch_plan(monkeypatch):
    recorder = _patch_dispatch(monkeypatch, {
...
# ---------------------------------------------------------------------------
# Selection-context defaulting (Canvas co-pilot wiring)
# ---------------------------------------------------------------------------

_CANVAS_CTX = [{
    "kind": chat_tools.CANVAS_CONTEXT_KIND,
    "workflow_id": "wf_active",
    "workflow_name": "canvas-demo",
...
def test_extract_canvas_context_finds_entry_among_other_selection_items():
    ctx = [
        {"kind": "row", "id": 1},
        {"kind": chat_tools.CANVAS_CONTEXT_KIND, "workflow_id": "wf_x"},
        {"kind": "row", "id": 2},
    ]
    found = chat_tools._extract_canvas_context(ctx)
    assert found is not None
    assert found["workflow_id"] == "wf_x"


def test_extract_canvas_context_returns_none_when_absent():
    assert chat_tools._extract_canvas_context(None) is None
    assert chat_tools._extract_canvas_context([]) is None
    assert chat_tools._extract_canvas_context([{"kind": "row"}]) is None


def test_resolve_workflow_id_prefers_explicit_args_over_context():
    workflow_id, from_ctx = chat_tools._resolve_workflow_id(
        {"workflow_id": "wf_explicit"}, _CANVAS_CTX
    )
    assert workflow_id == "wf_explicit"
    assert from_ctx is False


def test_resolve_workflow_id_falls_back_to_canvas_context():
    workflow_id, from_ctx = chat_tools._resolve_workflow_id({}, _CANVAS_CTX)
    assert workflow_id == "wf_active"
    assert from_ctx is True


def test_canvas_get_build_default_targets_active_workflow_from_context(monkeypatch):
    recorder = _patch_dispatch(monkeypatch, {
        "workflow_build_get": {
            "workflow_id": "wf_active",
            "name": "canvas-demo",
...
def test_canvas_get_build_reconciles_visible_ui_snapshot_when_persisted_read_is_empty(monkeypatch):
    recorder = _patch_dispatch(monkeypatch, {
        "workflow_build_get": {
            "workflow_id": "wf_active",
            "name": "canvas-demo",
...
def test_canvas_get_build_errors_when_no_explicit_id_and_no_context(monkeypatch):
...
def test_canvas_mutate_field_default_targets_workflow_from_context(monkeypatch):
    recorder = _patch_dispatch(monkeypatch, {
        "workflow_build.mutate": {
            "workflow_id": "wf_active",
            "build_graph": {"nodes": [], "edges": []},
...
def test_canvas_suggest_next_default_anchors_on_selected_node(monkeypatch):
    recorder = _patch_dispatch(monkeypatch, {
    ...
    assert recorder.calls[0][1]["workflow_id"] == "wf_active"
    # suggest_next anchored on the selected node from canvas_context
...
def test_canvas_launch_default_targets_workflow_from_context(monkeypatch):
    recorder = _patch_dispatch(monkeypatch, {
...
def test_chat_tools_registers_all_five_canvas_entries():
    names = {t["name"] for t in chat_tools.CHAT_TOOLS}
    assert {"canvas_get_build", "canvas_compose", "canvas_mutate_field", "canvas_suggest_next", "canvas_launch"} <= names


def test_canvas_tool_schemas_do_not_force_workflow_id_when_context_can_target_canvas():
    by_name = {t["name"]: t for t in chat_tools.CHAT_TOOLS}
    assert by_name["canvas_get_build"]["input_schema"]["required"] == []
    assert "workflow_id" not in by_name["canvas_mutate_field"]["input_schema"]["required"]
    assert by_name["canvas_suggest_next"]["input_schema"]["required"] == []
    assert by_name["canvas_launch"]["input_schema"]["required"] == []
