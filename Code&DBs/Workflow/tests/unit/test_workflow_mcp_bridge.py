...
def test_augment_cli_command_prefer_docker_normalizes_mcp_host(monkeypatch) -> None:
    import registry.provider_execution_registry as provider_registry_mod

    monkeypatch.setenv(
        "PRAXIS_WORKFLOW_MCP_URL",
        "http://localhost:8780/mcp?existing=1",
    )
    monkeypatch.setenv("PRAXIS_WORKFLOW_MCP_SIGNING_SECRET", "test-secret")
    monkeypatch.setattr(
        provider_registry_mod,
    ...
    assert parsed.port == 8780


def test_augment_cli_command_prefers_not_normalizing_when_not_in_docker(monkeypatch) -> None:
    import registry.provider_execution_registry as provider_registry_mod
