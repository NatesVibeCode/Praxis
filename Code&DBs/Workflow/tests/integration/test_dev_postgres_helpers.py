...
def _assert_native_postgres_disabled(exc: dev_postgres.DevPostgresError) -> None:
    assert exc.reason_code == "dev_postgres.disabled"
    assert "Native repo-local Postgres helpers are disabled" in str(exc)
...
def test_local_postgres_health_and_bootstrap_fail_closed_on_database_url_cluster_mismatch(
    ...
    _assert_native_postgres_disabled(exc_info.value)
...
def test_local_postgres_health_and_bootstrap_path_are_explicit_and_idempotent() -> None:
    env = _workflow_env()
    for helper in (local_postgres_health, local_postgres_bootstrap, local_postgres_status):
        with pytest.raises(dev_postgres.DevPostgresError) as exc_info:
            helper(env=env)
        _assert_native_postgres_disabled(exc_info.value)
