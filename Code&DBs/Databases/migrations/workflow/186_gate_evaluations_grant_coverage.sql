...
BEGIN
    IF NOT EXISTS (
        ...
        ALTER TABLE gate_evaluations
            ...
            REFERENCES mobile_capability_grants (grant_id)
            ON DELETE RESTRICT
            NOT VALID;
    END IF;
END $$;
...
COMMENT ON TABLE gate_evaluations IS
    'Canonical gate evaluation rows for sealed proposals, including mobile capability grant coverage when present.';

COMMENT ON COLUMN gate_evaluations.grant_ref IS
    'Mobile capability grant id that covered this gate evaluation, when one was active.';
