-- Migration 239: register_operation_atomic SQL helper.
--
-- Context: BUG-2062CB3B. Registering a new operation in the catalog requires
-- three rows landed in this order:
--   1. data_dictionary_objects (object_kind='operation.<name>', ...)
--   2. authority_object_registry (object_ref='operation.<name>',
--                                  data_dictionary_object_kind='operation.<name>', ...)
--   3. operation_catalog_registry (handler_ref, input_model_ref, ...)
--
-- The chain is enforced by CHECK triggers but undocumented; authors of
-- migrations 234, 236, etc. discovered the dependency only by hitting the
-- CHECK violations one at a time. This helper bundles the chain into one
-- function call so future migrations write a single SELECT and the trigger
-- chain stays internal.
--
-- Usage from a migration:
--   SELECT register_operation_atomic(
--       p_operation_ref         := 'my-op',
--       p_operation_name        := 'my_op',
--       p_handler_ref           := 'runtime.operations.commands.my_op.handle',
--       p_input_model_ref       := 'runtime.operations.commands.my_op.MyOpCommand',
--       p_authority_domain_ref  := 'authority.workflow_runs',
--       p_event_type            := 'my.event',
...
CREATE OR REPLACE FUNCTION register_operation_atomic(
    ...
    p_event_required           BOOLEAN DEFAULT FALSE,
...
COMMENT ON FUNCTION register_operation_atomic IS
    'Atomic operation-catalog registration that wraps the data_dictionary_objects '
    '-> authority_object_registry -> operation_catalog_registry chain so callers '
    'do not have to discover the CHECK trigger dependencies one violation at a '
    'time (BUG-2062CB3B). Use from migrations as: SELECT register_operation_atomic(...);';

COMMENT ON TABLE operation_catalog_registry IS
    'Canonical registry for operation definitions. This is the durable authority '
    'for operation identity, transport binding, and owning authority seams. '
    'REGISTRATION CHAIN: an enabled operation row requires (1) a matching '
    'data_dictionary_objects row keyed object_kind=''operation.<name>'' and '
    '(2) a matching authority_object_registry row keyed object_ref=''operation.<name>''. '
    'CHECK triggers enforce both. Use register_operation_atomic(...) to land all '
    'three rows in one call. See migrations 234, 236 for worked examples and '
    'BUG-2062CB3B for the friction this helper resolved.';
