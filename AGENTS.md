...
2. **Per-harness, raw tool path (Claude has it; Codex/Gemini docs in
   `policy/HARNESS_INTEGRATION.md`).** Bash / Edit / Write / Read calls
   that don't go through Praxis MCP get the same matcher applied via the
   harness's PreToolUse hook (Claude Code) or wrapper (Codex / Gemini).
   Surfaces standing orders as `additionalContext` (or harness equivalent)
   right before the tool runs.

The trigger registry lives at `policy/operator-decision-triggers.json`.
It maps each `operator_decisions.decision_key` to one or more match
conditions (regex on Bash command, file glob on Edit path, string match
on file content). The matcher itself is in
