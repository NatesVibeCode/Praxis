#!/usr/bin/env python3
"""Generate the Canvas UI Phase 1 spec with inlined source contents.
...
CANVAS_BUILD_PAGE = read_src("Code&DBs/Workflow/surfaces/app/src/canvas/CanvasBuildPage.tsx")
...
ART = "artifacts/canvas_ui_phase1_20260417"

spec = {
    "name": "Canvas UI Phase One — Spec + Diff (embedded source)",
    "workflow_id": "canvas_ui_phase1_20260417",
    "phase": "execute",
    "workdir": str(REPO),
    "queue_id": "canvas_ui_phase1_20260417",
    "outcome_goal": "Produce (a) architectural decomposition for CanvasBuildPage.tsx and (b) three typechecked, tested diffs against ToolResultRenderer.tsx, ChatPanel.tsx+useChat.ts, and MarkdownRenderer.tsx.",
    "jobs": [
        {
            "label": "analyze_monolith",
            "agent": "anthropic/claude-sonnet-4-6",
            "task_type": "architecture",
            "description": "Analyze CanvasBuildPage.tsx — layout regions, state ownership, coupling",
            "outcome_goal": "Written analysis with regions, state map, coupling surfaces sections.",
            "prompt": f"""Analyze the CanvasBuildPage.tsx React component below. Produce a structured architectural analysis.
...
Source file `surfaces/app/src/canvas/CanvasBuildPage.tsx` ({len(CANVAS_BUILD_PAGE.splitlines())} lines):
...
""",
        ...
        },
        {
            ...
            "prompt": f"""Propose a concrete decomposition of CanvasBuildPage.tsx. The prior job's analysis is at `{ART}/01_analysis.md` — read it if accessible, otherwise work from the source below.
...
**Proposed Subcomponents** — 4-8 subcomponents. For each: Name, Responsibility (one sentence), TypeScript Props interface (real types), Internal state, target file path like `surfaces/app/src/canvas/components/CanvasActionDock.tsx`.
...
Source file for reference ({len(CANVAS_BUILD_PAGE.splitlines())} lines):
...
Source file for cross-reference ({len(CANVAS_BUILD_PAGE.splitlines())} lines):
...
""",
        ...
        },
        {
            ...
            "description": "File P3 bug for CanvasBuildPage decomposition execution",
            "outcome_goal": "One P3 bug filed linking the decomposition plan as evidence.",
            "prompt": f"""File a single P3 bug in Praxis.db tracking the CanvasBuildPage.tsx decomposition work.
...
The bug to file (one):
- Title: `Execute CanvasBuildPage.tsx decomposition per canvas_ui_phase1_20260417 plan`
- Severity: `P3`
- Category: `UI`
...
Required sections:
```
# Canvas UI Phase One — Decision Record
...
out_path = REPO / ART / "canvas_ui_phase1.queue.json"
