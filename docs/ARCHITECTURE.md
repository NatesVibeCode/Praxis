...
SQL migrations in `Code&DBs/Databases/migrations/workflow/` (001-028). Each migration is idempotent and runs in sequence.
...
Embeddings are generated by `sentence-transformers` and stored as `vector(384)` or `vector(768)` columns.
