# Canvas UI Action Audit

Updated source audit for the Canvas builder as of 2026-04-15.

## Scope
- Frontend: `Code&DBs/Workflow/surfaces/app/src/canvas/*`
...
- `build_graph.edges[].release` is now the canonical edge-release authority for Canvas, build projection, planner, and runtime mapping.
...
- Empty connections expose the only two inline gate actions Canvas trusts today: `Branch` and `On Failure`.
...
- Canvas primitive metadata now lives in `surface_catalog_registry`; `/api/catalog` reads the table instead of carrying a Python-owned primitive list.
...
- Canvas no longer carries a browser-owned fallback copy of the action catalog; `/api/catalog` is the only authority for the primary action inventory now.
...
## Summary
- The old first-pass assumption that most Canvas node actions were decorative is no longer accurate.
...
- Product cut for the main Canvas surface:
...
| --- | --- |
| `+ New Operating Model` | Opens Canvas builder in operating-model flow. |
...
| dock half-canvass | Open or close docks only. |
...
| gate pod | Selects a connection and shows the inline core gate actions Canvas trusts today. |
