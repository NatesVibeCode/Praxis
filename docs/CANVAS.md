...
Canvas is the canonical operator UI for Praxis — one canvas, one chat panel, five surfaces. Every interaction persists: there is no separate "save" step, no sync lag, no state that lives only in the browser. What you see in Canvas is what Praxis.db contains.

For the trust-compiler and term concepts behind what Canvas shows you, see [CONCEPTS.md](CONCEPTS.md).
